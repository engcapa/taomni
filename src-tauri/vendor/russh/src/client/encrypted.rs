    ...
    pub(crate) fn write_auth_request_if_needed(&mut self, user: &str, meth: auth::Method) -> bool {
        ...
        if let Some(ref mut enc) = self.common.encrypted {
            is_waiting = match enc.state {
                EncryptedState::WaitingAuthRequest(ref mut auth_request) => {
                    // When keyboard-interactive is started after another method
                    // (e.g. password partial-success), set current so the
                    // subsequent USERAUTH_INFO_REQUEST is parsed as KI instead
                    // of being misrouted as a pubkey-OK probe and silently
                    // dropped. Without this, MFA prompts are never delivered.
                    if let auth::Method::KeyboardInteractive { ref submethods } = meth {
                        auth_request.current =
