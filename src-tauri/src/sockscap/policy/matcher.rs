...
use super::rules::{CompiledRule, RuleKind};
...
impl ProfileMatcher {
    ...
    pub fn decide(&self, input: &FlowMatchInput) -> PolicyDecision {
        ...
        if host.is_none()
            && ip.is_none()
            && matches!(input.hostname_source, HostnameSource::Unknown)
        {
            return PolicyDecision {
        ...
        // 5. Default
        // If we only have IP and nothing matched, still use default_action.
        // If hostname is unknown-ish, unknown_domain_action already handled above.
        let action = if host.is_none()
            && matches!(
                input.hostname_source,
                HostnameSource::Unknown | HostnameSource::IpOnly
            ) {
            // IP-only flows use default unless unknown was forced.
            if matches!(input.hostname_source, HostnameSource::Unknown) {
                self.unknown_domain_action
            } else {
                self.default_action
            }
        } else {
        ...
        PolicyDecision {
            ...
            matched_rule_original: None,
            matched_rule_source_id: None,
            matched_stage: "default_action".into(),
            hostname_source: input.hostname_source,
            profile_id: self.profile_id.clone(),
        }
    }
