...
mod tests {
    ...
    #[test]
    fn redacts_password() {
        let (r, n) = redact("mysql -u root password=secret123");
        assert!(r.contains("[REDACTED]"));
        assert!(n > 0);
    }
    #[test]
    fn redacts_bearer() {
        let (r, n) = redact("Authorization: Bearer eyJhbGc.abc");
        assert!(r.contains("[REDACTED]"));
        assert!(n > 0);
    }
    #[test]
    fn safe_text() {
        let (r, n) = redact("ls -la /home/user");
        assert_eq!(r, "ls -la /home/user");
        assert_eq!(n, 0);
    }
}
