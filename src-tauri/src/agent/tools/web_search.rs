...
impl Tool for WebSearchTool {
    ...
    async fn execute(&self, args: &serde_json::Value) -> ToolResult {
        ...
        let freshness = args.get("freshness").and_then(|v| v.as_str()).map(|s| s.to_string());
        let max_results = args.get("max_results").and_then(|v| v.as_u64()).unwrap_or(5) as u32;

        let opts = SearchOptions { freshness, max_results: max_results.min(10) };
        ...
        match self.provider.search(&query, &opts).await {
            Ok(hits) => {
                ...
                ToolResult::ok("web_search", if formatted.is_empty() {
