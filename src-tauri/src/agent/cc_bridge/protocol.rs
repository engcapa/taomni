...
pub fn parse_ndjson_line(line: &str) -> Option<CcEvent> {
    ...
    match event_type {
        ...
        "tool_result" => {
            ...
            Some(CcEvent::ToolResult { tool_use_id, content })
        }
        "result" => {
            // CC final result event
            if value.get("subtype").and_then(|v| v.as_str()) == Some("error") {
                let msg = value.get("error").and_then(|v| v.as_str()).unwrap_or("Unknown error").to_string();
        ...
        "error" => {
            let msg = value.get("message").and_then(|v| v.as_str()).unwrap_or("Unknown error").to_string();
            Some(CcEvent::Error { message: msg })
        }
        _ => Some(CcEvent::Unknown { raw: line.to_string() }),
    }
}
