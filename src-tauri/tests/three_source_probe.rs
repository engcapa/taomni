...
fn manifest_has_three_sources_per_entry() {
    ...
    assert!(!manifest.models.is_empty(), "manifest must list at least one model");
...
fn manifest_includes_asr_and_llm_kinds() {
    ...
    let manifest = load_manifest().expect("models manifest must load");
    let has_asr = manifest.models.values().any(|m| matches!(m.kind, ModelKind::Asr));
    let has_llm = manifest.models.values().any(|m| matches!(m.kind, ModelKind::Llm));
