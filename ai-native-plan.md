  ...
  "llm": {
    ...
    "providers": {
      ...
      "claude-code": { "runtime": "claude-cli", "binary": "auto",      "default_model": "sonnet",                "permission_mode": "default", "max_turns": 20 }
                     // 用户独立安装的 Claude Code CLI；NewMob 不打包二进制 / 不存凭证。详见 §九.5
    },
...
**目标**：在 NewMob 现有终端输入辅助系统上增量加 AI。现有基线：
...
**目标**：给 Agent 加上"上网查最新资料"的能力——例如查 kubectl 1.32 的 release notes、查某个错误码在 GitHub Issues 中的讨论、查 OpenSSH 9.x 新语法。主流商业 LLM Provider 大多已内置 web_search（OpenAI / Claude / Gemini / Grok / Mistral / GLM / Qwen / Perplexity Sonar），NewMob 走 capability-aware 双轨——优先用 Provider 原生（更隐私、更省钱、更快），客户端 SearXNG/Tavily 作为深度搜索（`deep_search`）补位。
...
**目标**：把"重型 agentic"能力——多文件代码理解、长上下文规划、跨工具编排——外包给用户独立安装的 Claude Code CLI（claude.ai/code）。NewMob **不打包** Claude Code 二进制、**不存** Anthropic 凭证；用户已 `claude login` 或配好 `ANTHROPIC_API_KEY` / Bedrock / Vertex 后，NewMob 检测到即把"Claude Code"作为一个 Provider 选项暴露在 Drawer 顶部切换器里。

不替代 §七 的 rig-core 路径——两者并存：

| 后端 | 定位 | 适用场景 | 默认 |
|------|------|---------|------|
| **rig-core**（§七 已规划） | 单工具 / ≤5 步 / 低延迟 / 与 NewMob 工具紧耦合 | 语音意图、命令生成、报错解释、Drawer 默认 chat | ✅ |
| **Claude Code CLI**（v2.6 新增） | 多文件代码 / 长上下文 / 项目级 plan-then-execute | "代码模式"开关、复杂 runbook 生成、远端代码审查 | ❌ opt-in |

CC 路径补的是 NewMob 自己短期内做不好的——多步规划、长上下文、project-aware tool use。让用户复用自己已订阅的 Claude Pro/Max 或已配置的 Anthropic 额度，无侵入。

### 9.5.1 用户流程

1. 设置 → AI → 高级后端 → 启用"Claude Code 集成"（默认关闭）
2. NewMob 调用 `claude --version`：
...
4. 用户在某 thread 选 Claude Code → NewMob spawn 长子进程 → 后续消息走 stream-json stdin / stdout → CC 调用工具时弹 NewMob 自己的 ActionCard 确认 → 用户允许后 CC 继续推进
...
- **不打包二进制**：检测不到时仅显示安装指引；NewMob installer 体积零增量
...
- **权限 UI 收口到 NewMob**：用 `--permission-prompt-tool <our_mcp_tool>` 把 CC 每次工具调用的批准弹到 v2.1 命令预览卡 / v2.3 ActionCard，用户感知不到是 CC 在驱动
- **NewMob 工具反向暴露**：把 §7.1 的 9 个 tool 同时实现一份 rmcp wrapper（v2.3 已经在 `agent/mcp_server.rs` 有了，复用），通过 `--mcp-config` 注入。CC 也能 `switch_tab` / `run_in_terminal(dry_run)` / `sftp_upload` / `read_terminal_tail`
  ...
  "providers": {
    "claude-code": {
      ...
      "permission_mode":   "default",       // default | acceptEdits | plan
                                            // 不允许 bypassPermissions（NewMob 强制有人审）
...
```
CC 子进程                NewMob permission_prompt MCP             用户
   │                              │                               │
...
CC 的"写"工具（Edit / Write / Bash）走同一条审批通道；NewMob 黑名单（§5.3）在 permission_prompt 层强制生效，CC 即使 plan-mode 编排到 `rm -rf /` 也会被同样的正则拦下。

### 9.5.5 工具反向暴露 + 隔离

- `tools_mcp.rs` 暴露的 9 个工具与 §七 rig-core 共用同一组 Rust 实现（同 `safety.rs` 中间件），仅多包一层 rmcp 协议适配。`dry_run=true` 默认行为不变
- CC 自带的 `Read` / `Edit` / `Bash` / `Glob` / `Grep` 与 NewMob 工具**并存**，NewMob 不禁用它们——但通过 `--add-dir` 白名单限制 CC 只能看 thread 关联工作区，**绝不允许** CC 自己 walk `~/.ssh` / vault 目录 / NewMob 配置目录
- CC 的 web search 由 Anthropic 原生提供，**不**注册 NewMob 的 `web_search` / `deep_search`（§9.0 的 ProviderCaps 抽象增加 `ClaudeCode { native_search: true }` 一行即可）
...
- *stream-json schema 漂移* → CC 仍快速迭代，pin 一个 `min_version`（启动检测时拒绝过老版本，提示升级）；NewMob 自己跟踪 changelog
- *长 stdio 子进程在 Windows stdin pipe 关闭时 quirky* → watchdog + 自动重启 + 流式状态恢复（与 §四 4.7 sidecar 复用 watchdog）
- *CC 偷偷读敏感目录* → `--add-dir` 白名单 + 启动前生成临时 settings.json `deny: ["~/.ssh/**", "<vault dir>/**", "<newmob config>/**"]`；CC 越界访问的 tool_call 在 permission_prompt 层直接拒绝
...
- *CC OAuth 凭证文件被 NewMob 误读* → 严禁；`cc_bridge` 模块禁止 `use crate::vault::*`，编译期 lint（与 §四 4.4 同款约束）
...
- **不读 / 不存 Anthropic 凭证** —— 完全交给 CC 自己；NewMob 进程内不持有任何 `ANTHROPIC_*` env 副本超过 spawn 那一瞬
- **不允许 `--permission-mode bypassPermissions`** —— 该模式跳过所有审批，与 NewMob "写动作必有人审"原则冲突
- **不让 CC 替代 §六 ghost-text / §2.2 FIM** —— 延迟预算不允许；CC 只服务 Drawer 多步任务
- **不向 CC 暴露 vault 工具 / 不允许 CC 调用 NewMob 的 Vault 解锁 API** —— 即便 CC 的 ActionCard 被批准，密钥材料也不进任何 LLM 上下文（继承 §零的铁律）
...
不用 HuggingFace 主源的原因：HF 在国内经 hf-mirror.com 也是只读 passthrough，不能让 NewMob 上传；ModelScope 由阿里云持有、允许任意账户 `git push` LFS 仓库。HF 仅作"海外可达性最强备选"，不进默认 manifest 顺序。
...
CUDA pack 不打包进 installer：避免分发 200–400 MB 的 CUDA 运行时与法律审查。用户主动开启时下载到 `~/.cache/newmob/sidecar-cuda/llama-server-cuda-{triple}`，运行时用 `tauri-plugin-shell::Command::new`（不是 sidecar——sidecar 强制 build-time 路径）。
...
32. **读 / 存 Anthropic 凭证** —— CC 自己拿凭证；NewMob 进程内不持有任何 `ANTHROPIC_*` env 副本超过 spawn 那一瞬
33. **CC `--permission-mode bypassPermissions`** —— 该模式跳过所有审批，与"写动作必有人审"原则冲突；NewMob 仅允许 `default | acceptEdits | plan`
...
- 已装 + 已认证：选 Claude Code Provider 后，Drawer 中输入"分析 src/components/terminal/TerminalPanel.tsx 的事件流" → CC 调用 `Read` / `Grep` → 每次工具弹 NewMob ActionCard → 用户允许 → 流式回答
...
- 关闭 NewMob → 重开同一 thread → 用 `cc_session_id` `--resume`，CC 上下文不丢
- 子进程 kill 模拟：watchdog 在 1 s 内重启；连续 3 次失败 → 该 thread 切回 fallback.secondary（DeepSeek 或本地）
- `cargo test -p newmob_agent cc_bridge::vault_isolation` —— 编译期 lint：`cc_bridge::*` 不允许 `use crate::vault::*`
...
- ✅ **`permissions_mcp.rs` 完整收口**（`agent/cc_bridge/permissions_mcp.rs` 实现 stdio NDJSON MCP server；CC spawn 时附 `--mcp-config <temp.json>` + `--permission-prompt-tool mcp__newmob_permissions__permission_prompt`，CC 每次工具调用都先经 NewMob safety pipeline 审批）
...
- ✅ **CUDA pack 按需下载入口**（`cuda_pack_status` / `cuda_pack_install` / `cuda_pack_uninstall` 命令 + ModelsAdvancedPanel UI；下载到 `<cache>/newmob/sidecar-cuda/`，`gpu_detect::detect_cuda_pack` 自动识别）
...
- ✅ **性能基线持久化**（`src-tauri/src/perf.rs` `LatencyMetric` + `record_now` + JSONL 落地 `<cache>/newmob/perf-baseline.jsonl`；`perf_baseline_recent` 命令供前端读取）
