...
需覆盖语音→Shell、Tab 补全 FIM、AI Chat Drawer、终端 `??` 内联对话、文本选区 Send-to-AI 等综合场景，因此除 code 任务外还要看 MMLU / CMMLU / IFEval / BFCL（tool-calling）的综合表现。
...
| 任务 | 模型 | 选择理由 |
|------|------|---------|
| 默认 chat（含语音→Shell、AI Chat Drawer、`??` 内联）| **Qwen3-1.7B-Instruct-2507** | 1.4 GB；MMLU 62.6 / MMMLU 63.3 / MGSM 47.5；原生 tool-calling；非 thinking 模式可关，延迟无惩罚 |
| Tab 补全 FIM | **Qwen3-0.6B-Instruct** | 同体积下推理质量优于 Qwen2.5-Coder-0.5B；Apache-2.0 |
...
**不进路线的候选**：Llama 3.2（中文不在官方 8 语言）、Gemma 3-1B（英语为主）、Phi-3.5-mini / Phi-4-mini（CMMLU ~46.9）、SmolLM2 / SmolLM3（英语为主）、DeepSeek-R1-Distill-1.5B（无 tool-calling）、Yi-Coder（仅代码）。

CPU-only 现实：Qwen3-1.7B 在 i5/Ryzen5 上首 token ~300 ms、~25–35 tok/s；意图解析（~50 token JSON）端到端 ~500–1000 ms；语音→Shell 含 ASR 总耗时 ~700–1200 ms。chat 场景中长回答仍可能拉到 5–10 s——故 chat 默认走云端 LLM，本地仅用于 FIM 与短意图。详见 §1.5。

### 1.2 本地 ASR（中英双语，必须本地）

短命令（1–10 s 音频）的 PTT 场景实测数据：
...
默认锁定：sherpa-onnx 流式 Zipformer zh-en small（中文 CER ~10–15%，真流式 partial）。whisper-rs 仅作 feature flag 备用。zh-pro 档用 SenseVoice-Small（部分权重为 CC-NC 限商用，需在 manifest 标注，详见末尾注）。Whisper-tiny 在中文上 CER 65–71%，不适合短命令场景。
...
1. **llama.cpp `llama-server` sidecar（v2.0 主选）** ⭐
   - 通过 Tauri `bundle.externalBin` 打包 `llama-server` 二进制：CPU-only 版本 ~8–15 MB / 平台
   - OpenAI 兼容（`/v1/chat/completions`、`/v1/completions`），并提供原生 `/infill` 端点（含 `--fim-qwen-*` 预设）做 FIM
   - 模型管理需自实现：从 ModelScope / GitHub Releases 下载 GGUF + SHA-256 校验（与 ASR 模型库共用基础设施，详见 §十一）
...
2. **`llama-cpp-2` crate（v2.2 in-process FIM 用）** ⭐
   ...
   - 与 `whisper-rs` / `sherpa-onnx`（如启用 ggml backend）通过构建脚本共用 system ggml，避免符号冲突
   - 构建依赖 clang，Windows CI 需配置；API 是 unsafe 的低层接口，外覆 wrapper

3. **Ollama（用户已装时可接入）**
   - 用户本机已装 Ollama 时，作为 OpenAI 兼容 Provider 的 base_url 之一接入（设置 → AI → 添加 Provider → Ollama 模板，自动探测 `127.0.0.1:11434`）

4. **mistral.rs / candle（远期备选）**
   - 完全 in-process，无 sidecar，但 GGUF 加载性能弱于 llama.cpp，FIM 需手动构造 prompt
   - 留作 v2.4+ 评估，不进 v2.x 默认路径

v2.0 锁定 `llama-server` sidecar；v2.2 Tab 补全切到 `llama-cpp-2` in-process。所有路径走同一 Provider trait（仅 base_url / 进程模型不同），UX 上对用户透明。
...
// ~/.config/taomni/ai.json
{
  ...
  "llm": {
    ...
    "providers": {
      ...
      "claude-code": { "runtime": "claude-cli", "binary": "auto",      "default_model": "sonnet",                "permission_mode": "default", "max_turns": 20 }
                     // 用户独立安装的 Claude Code CLI；Taomni 不打包二进制 / 不存凭证。详见 §九.5
    },
...
> **隐私一键开关**：设置顶部提供"全本地模式"开关。打开后 `llm.task_routing` 全部强制写为 `local`，禁用所有云端 Provider 的网络请求（Tauri allowlist 动态收紧），并隐藏 Claude Code 集成（CC 必走 Anthropic 云端，详见 §九.5）。配合本地模型库下载完成后，整个应用可断网工作。

API Key 通过 vault `kind=ai_api_key:<provider>` 存储，复用 v1.1 已建立的机制。ASR 无 api_key 概念，故不入 vault。

### 1.6 超时回退（云端 8 s → 本地）

默认云端为 primary，本地为 secondary。用户网络异常或 API Key 失效时自动切回本地（前提：本地档位已下载）。
...
|------|-----|-------------|------------|--------|---------|-------------|------|
| **cloud-only** ⭐ 默认 | sherpa Zipformer 80 MB | 云端 | — | **80 MB** | ~200 MB | ~250 MB | 大多数用户首选；FIM 不开（Taomni 现有 inline ghost-text 已可用） |
...
铁律：**安装包不打包任何模型**（既不打 ASR 也不打 LLM 也不打 sidecar 二进制）。安装后 Taomni 恢复"纯终端 + SSH/SFTP/VNC"形态；任何 AI 功能首次启用时才提示下载。

| 用户动作 | 触发的下载 | 大小 | 提示形式 |
|---------|-----------|------|---------|
| 全新安装 Taomni | 0 字节 | 0 MB | — |
...
总占用: 1.89 GB · cache 路径: ~/.cache/taomni/models/
[全部删除] 重置为 cloud-only
```
...
# —— v2.2 输入辅助（in-process FIM；不再使用 reedline，复用现有 ghost-text 渲染层）——
...
> `rig-core` pre-1.0，0.27 → 0.37 间存在 API 变更。在 `src-tauri/src/agent/mod.rs` 内做一层薄 wrapper（约 50–100 行），未来切换 swiftide-agents 或 0.4x 时只动 wrapper。
...
```jsonc
{
  "tauri": {
    "bundle": {
      "externalBin": [
        "binaries/llama-server"          // CPU + Vulkan 二合一单二进制
                                          // CUDA pack 不打包进 installer，按需下载到 ~/.cache/taomni/sidecar-cuda/
                                          // 详见 §十一 GPU 策略
      ],
      "resources": [
        "resources/models.manifest.json"  // 模型分发清单（~5–10 KB），含三源 URL；详见 §十一
      ]
    },
    "allowlist": {
...
## 四、v2.0 —— ASR Trait + LLM Provider 抽象（双解耦）

**目标**：在 v1.x 已有 `AiBackend` 基础上拆分出两个独立 trait：
...
解决：build.rs 中设置 `WHISPER_USE_SYSTEM_GGML=1`、`LLAMA_USE_SYSTEM_GGML=1`，并显式提供一份 system ggml（通过 `ggml-sys` 或自构建）。
...
## 五、v2.1 —— 语音 → Shell 命令 / 脚本生成

**目标**：在 v1.x"只读"路由之外开放"执行 shell"路径，以"强守门"形态。整个特性默认关闭，开启时弹一次性的安全说明书。
...
- *AI 在中文 ffmpeg / docker 这类长尾场景胡编参数* → 优先走云端大模型（DeepSeek/Qwen-Coder-Plus）；本地失败时自动回退云端
...
**目标**：在 Taomni 现有终端输入辅助系统上增量加 AI。现有基线：
- **Fish 风格 inline ghost-text**：`src/components/terminal/TerminalPanel.tsx:351–373`，按键时根据历史前缀匹配显示灰色补全；`Tab` 透传给 shell。
...
v2.2 的工作是**在已有的 ghost-text 渲染层上叠加新数据源**。
...
**FIM 选 in-process 而非 sidecar**：HTTP 往返多 30–80 ms（loopback + JSON 编解码 + tokio 跨进程调度），加上 debounce 后预算就紧。`llama-cpp-2` 直接 FFI 调用，零 IPC 开销。

### 6.4 `Ctrl+K`：AI 命令改写专用入口
...
- *PowerShell 例外* → 保持现状，PSReadLine 自身预测足够好，不再叠加 Taomni ghost-text

---

## 七、v2.3 —— rig-core Agent harness + MCP 暴露
...
| 路径 | 选择 | 理由 |
|------|-----|------|
| **主选 harness** | `rig-core` v0.37（pin 精确版本） | `#[derive(Tool)]` 自动 JSON schema；内置多步 ReAct + max_iterations；Provider 覆盖 OpenAI / Anthropic / DeepSeek / Groq / Gemini / Ollama / OpenAI-compat（涵盖 llama-server）；MIT 许可 |
| **备选 harness** | `swiftide-agents` | rig 中间件钩子（dry-run / 安全检查 / 审计）不够用时切换；它原生提供 `before_tool` / `after_tool` / `before_completion` 生命周期 |
| **MCP 暴露** | `rmcp` v1（官方 SDK） | 把 Taomni 的工具同时暴露为 MCP server，让 Claude Desktop / Goose / Cursor 等也能驱动 Taomni 的会话切换、命令注入。v2.0 内部路径优先，v2.3 加 MCP server 层 |
| **不走 PyO3** | — | PydanticAI / smolagents 优势在 Logfire 等观测栈，与 Taomni 的 5 步 ReAct + 6 工具规模不匹配；+30–60 MB Python 体积、GIL、跨平台签名都太重 |
...
**技术风险**（重点）：
- xterm.js 与 PTY 数据流的拦截层目前 Taomni 走 `write_terminal(base64)`，需要在前端先做"未发送行缓冲"
...
xterm.js 已有 `selectionChange` 事件。复用 Taomni 现有的右键菜单（`src/components/terminal/`），新增三个动作：
...
**目标**：给 Agent 加上"上网查最新资料"的能力——例如查 kubectl 1.32 的 release notes、查某个错误码在 GitHub Issues 中的讨论、查 OpenSSH 9.x 新语法。主流商业 LLM Provider 大多已内置 web_search（OpenAI / Claude / Gemini / Grok / Mistral / GLM / Qwen / Perplexity Sonar），Taomni 走 capability-aware 双轨——优先用 Provider 原生（更隐私、更省钱、更快），客户端 SearXNG/Tavily 作为深度搜索（`deep_search`）补位。

### 9.0 原生 vs 客户端搜索（capability-aware 路由）

原生方案 query 仅到一个第三方（vs 客户端方案两个），延迟少 0.5–1.5 s，结果质量也更好（直连 Bing/Google 索引）。但客户端方案是无原生 Provider（DeepSeek / 本地 / Groq / Cerebras）的唯一选择，也是 BYOK 用户偏好。

**Provider 原生能力矩阵**：

| Provider | 原生 web search | 计费（额外）| 引用机制 | Taomni 处理 |
|----------|----------------|------------|---------|------------|
...
|--------------|----------|---------|
| 有原生 + 用户开启 native | `web_search`（→ 透传到原生）+ `deep_search`（→ Taomni 客户端，仅 BYOK 时启用）| 默认走原生；用户主动 `--deep` 触发深度 |
| Perplexity Sonar | **不注册任何搜索工具** | Provider 自己搜 |
| 无原生（DeepSeek/Groq/本地）| 仅 `web_search`（→ Taomni 客户端 SearXNG）| 走客户端路径 |
...
| 维度 | 原生（Provider 内部）| 客户端（Taomni 自调）|
|------|-------------------|-------------------|
...
商业搜索 API 都需要 key，把 key 嵌入桌面二进制几小时内会被人提取并刷爆；Bing Search API 已于 2025-08 下线、Kagi API 仍是邀请制、DuckDuckGo Instant Answer 不返回普通 SERP。
...
公共实例白名单以"最近 30 天可用率"动态轮换：默认列表 `searx.be`、`search.inetol.net`、`searxng.world` 等；Taomni 启动时 ping 一遍，挑选可用的。
...
// 3. reqwest GET，带 User-Agent: "Taomni/x.y (web_fetch)"，10 s 超时
...
- *BYOK key 从二进制提取* → 不存在；Taomni 不嵌入任何默认 key；BYOK key 走 OS `keyring`（macOS Keychain / Windows Credential Vault / Linux Secret Service），不进 SQLite，不进 Taomni 进程内存超过必要时长
...
## 九.5 —— v2.6 Claude Code 高级后端（可选 / 用户自带 CLI）

**目标**：把"重型 agentic"能力——多文件代码理解、长上下文规划、跨工具编排——外包给用户独立安装的 Claude Code CLI（claude.ai/code）。Taomni **不打包** Claude Code 二进制、**不存** Anthropic 凭证；用户已 `claude login` 或配好 `ANTHROPIC_API_KEY` / Bedrock / Vertex 后，Taomni 检测到即把"Claude Code"作为一个 Provider 选项暴露在 Drawer 顶部切换器里。

不替代 §七 的 rig-core 路径——两者并存：

| 后端 | 定位 | 适用场景 | 默认 |
|------|------|---------|------|
| **rig-core**（§七 已规划） | 单工具 / ≤5 步 / 低延迟 / 与 Taomni 工具紧耦合 | 语音意图、命令生成、报错解释、Drawer 默认 chat | ✅ |
| **Claude Code CLI**（v2.6 新增） | 多文件代码 / 长上下文 / 项目级 plan-then-execute | "代码模式"开关、复杂 runbook 生成、远端代码审查 | ❌ opt-in |

CC 路径补的是 Taomni 自己短期内做不好的——多步规划、长上下文、project-aware tool use。让用户复用自己已订阅的 Claude Pro/Max 或已配置的 Anthropic 额度，无侵入。

### 9.5.1 用户流程

1. 设置 → AI → 高级后端 → 启用"Claude Code 集成"（默认关闭）
2. Taomni 调用 `claude --version`：
   - 未安装 → 显示一行说明 + 三个一键复制安装命令（Windows `winget install Anthropic.Claude`、macOS `brew install anthropic/claude/claude`、`npm i -g @anthropic-ai/claude-code`），不做静默安装
   - 已安装但版本低于最低支持 → 提示升级
   - 已安装 + 版本 OK → 验证认证状态（调一个最小 `claude -p "ping" --output-format json` 探针），失败则提示用户跑 `claude login`
3. 验证通过后，Drawer 顶部 Provider 切换器多一个选项 `Claude Code (本机)`；状态栏 LLM 段可点击切换
4. 用户在某 thread 选 Claude Code → Taomni spawn 长子进程 → 后续消息走 stream-json stdin / stdout → CC 调用工具时弹 Taomni 自己的 ActionCard 确认 → 用户允许后 CC 继续推进

### 9.5.2 接入架构

```
src-tauri/src/agent/cc_bridge/
  mod.rs              检测 claude --version、版本下限、缺失时降级为禁用
  process.rs          spawn `claude -p --output-format stream-json
                      --input-format stream-json --include-partial-messages`
                      持有 stdin / stdout pipe；watchdog + 自动重启
  protocol.rs         NDJSON 编解码；CC 事件归一到 ChatMessage / ActionCard
  permissions_mcp.rs  起一个 stdio MCP server，暴露 permission_prompt 工具
                      `--permission-prompt-tool` 把每次工具批准回调到 v2.3 ActionCard
  tools_mcp.rs        起第二个 stdio MCP server，把 §7.1 的工具反向暴露给 CC
                      复用 `agent::mcp_server.rs` 的 rmcp wrapper
  config.rs           生成临时 .mcp.json + 注入 --mcp-config <file>
                      生成临时 settings.json（permission-mode、allowed-tools 白名单）
  session.rs          ChatThread.cc_session_id 持久化；下次启动用 --resume
```

关键设计：

- **不打包二进制**：检测不到时仅显示安装指引；Taomni installer 体积零增量
- **认证完全外包**：宿主从不读 `ANTHROPIC_API_KEY`、不存 OAuth token；CC 自己拿凭证。这点与 §1.5"API Key 走 vault"路线相反——CC 是个例外，因为它的 token 跨工具共享，强行存我们的 vault 反而是 anti-pattern
- **权限 UI 收口到 Taomni**：用 `--permission-prompt-tool <our_mcp_tool>` 把 CC 每次工具调用的批准弹到 v2.1 命令预览卡 / v2.3 ActionCard，用户感知不到是 CC 在驱动
- **Taomni 工具反向暴露**：把 §7.1 的 9 个 tool 同时实现一份 rmcp wrapper（v2.3 已经在 `agent/mcp_server.rs` 有了，复用），通过 `--mcp-config` 注入。CC 也能 `switch_tab` / `run_in_terminal(dry_run)` / `sftp_upload` / `read_terminal_tail`
- **会话续接**：把 CC 的 `session_id` 存到 ChatStore 的 `cc_session_id` 字段，下次启动 thread 用 `--resume`，免得每轮重头规划
- **网络边界**：CC 自己走 Anthropic API（或用户配置的 Bedrock/Vertex）——流量在子进程里，不经 webview，Tauri http allowlist 不需要任何调整
- **隐私模式联动**：§十"全本地模式"开关打开时，CC Provider 选项整个隐藏（CC 必然云端调用 Anthropic）。这点必须明确，不能让"全本地"开关名不副实

### 9.5.3 配置 schema 增量（在 §1.5 之上）
...
"llm": {
  "providers": {
    "claude-code": {
      "runtime":           "claude-cli",
      "binary":            "auto",          // "auto" | "<absolute path>"
      "default_model":     "sonnet",        // 透传给 CC --model
      "permission_mode":   "default",       // default | acceptEdits | plan
                                            // 不允许 bypassPermissions（Taomni 强制有人审）
      "max_turns":         20,              // CC 单 thread 内多步上限
      "allowed_dirs":      ["${threadWorkspace}"], // --add-dir 白名单
      "min_version":       "1.0.0"
    }
  },
  "task_routing": {
    "code_mode":     "claude-code",         // Drawer "代码模式" 开关默认走 CC
    "agent_default": "deepseek"             // 其他 agent 任务保持云端 chat 默认
  }
}
```

任务路由原则：CC **不进** §6 ghost-text / Ctrl+K 改写 / 语音意图等延迟敏感路径（CC 在云端 + 长上下文，TTFT 通常 1.5–3 s）。仅用于：

- Drawer 顶部用户主动切到"Claude Code"的 thread
- v2.3 多步编排里用户主动开"代码模式"开关
- v2.4 选区 Send-to-AI 中"分析这段代码"的右键菜单子项

### 9.5.4 权限收口流程

```
CC 子进程                Taomni permission_prompt MCP             用户
   │                              │                               │
   ...
   │                              │                               │ 看 risk / 编辑 / 按 Enter
   │                              │◄──────────────────────────────┤
   │                              │ approve { command, dry_run }  │
   │ permission_decision           │                               │
   │◄─────────────────────────────┤                               │
   │ 继续执行                      │                               │
```

CC 的"写"工具（Edit / Write / Bash）走同一条审批通道；Taomni 黑名单（§5.3）在 permission_prompt 层强制生效，CC 即使 plan-mode 编排到 `rm -rf /` 也会被同样的正则拦下。

### 9.5.5 工具反向暴露 + 隔离

- `tools_mcp.rs` 暴露的 9 个工具与 §七 rig-core 共用同一组 Rust 实现（同 `safety.rs` 中间件），仅多包一层 rmcp 协议适配。`dry_run=true` 默认行为不变
- CC 自带的 `Read` / `Edit` / `Bash` / `Glob` / `Grep` 与 Taomni 工具**并存**，Taomni 不禁用它们——但通过 `--add-dir` 白名单限制 CC 只能看 thread 关联工作区，**绝不允许** CC 自己 walk `~/.ssh` / vault 目录 / Taomni 配置目录
- CC 的 web search 由 Anthropic 原生提供，**不**注册 Taomni 的 `web_search` / `deep_search`（§9.0 的 ProviderCaps 抽象增加 `ClaudeCode { native_search: true }` 一行即可）

### 9.5.6 检测、降级、平台

| 状态 | Taomni 行为 |
|------|------------|
| `claude --version` 未找到 | Provider 选项灰掉 + 一行安装指引；rig-core 路径不受影响 |
| 版本低于 `min_version` | 选项灰掉 + 提示升级 |
| 已安装但 `claude -p "ping"` 失败 | 选项可见但带黄点；点击展开"未认证，请在终端运行 `claude login`" |
| 子进程崩溃 | watchdog 自动重启；连续 3 次失败后该 thread 切回 §1.5 fallback.secondary（云端 DeepSeek 或本地 llama-server） |
| 全本地模式开启 | CC Provider 整个隐藏 |
| Windows native | `claude.cmd` / `claude.exe`（winget 或 npm 全局），不强制 WSL；`which` crate 已能定位 |

### 9.5.7 风险 + 缓解

- *stream-json schema 漂移* → CC 仍快速迭代，pin 一个 `min_version`（启动检测时拒绝过老版本，提示升级）；Taomni 自己跟踪 changelog
- *长 stdio 子进程在 Windows stdin pipe 关闭时 quirky* → watchdog + 自动重启 + 流式状态恢复（与 §四 4.7 sidecar 复用 watchdog）
- *CC 偷偷读敏感目录* → `--add-dir` 白名单 + 启动前生成临时 settings.json `deny: ["~/.ssh/**", "<vault dir>/**", "<taomni config>/**"]`；CC 越界访问的 tool_call 在 permission_prompt 层直接拒绝
- *CC 计费失控（用户订阅外的账户被刷）* → 顶部 Provider 标签显示当前模型 + 估算 token；`max_turns` 硬限；用户可设 thread 级 token budget
- *与 rig-core 体验割裂* → ActionCard / 命令预览卡 / 审计日志 schema 完全复用，用户在 Drawer 中切换 Provider 时只感觉"模型变了"
- *CC OAuth 凭证文件被 Taomni 误读* → 严禁；`cc_bridge` 模块禁止 `use crate::vault::*`，编译期 lint（与 §四 4.4 同款约束）

### 9.5.8 不做事项（v2.6 范围内）

下列条目同步并入 §十四 主清单（编号 31–35）：

- **不打包 Claude Code 二进制 / 不分发安装器** —— 仅检测 + 引导
- **不读 / 不存 Anthropic 凭证** —— 完全交给 CC 自己；Taomni 进程内不持有任何 `ANTHROPIC_*` env 副本超过 spawn 那一瞬
- **不允许 `--permission-mode bypassPermissions`** —— 该模式跳过所有审批，与 Taomni "写动作必有人审"原则冲突
- **不让 CC 替代 §六 ghost-text / §2.2 FIM** —— 延迟预算不允许；CC 只服务 Drawer 多步任务
- **不向 CC 暴露 vault 工具 / 不允许 CC 调用 Taomni 的 Vault 解锁 API** —— 即便 CC 的 ActionCard 被批准，密钥材料也不进任何 LLM 上下文（继承 §零的铁律）
...
**冲突避免**：
- xterm.js 中 `Ctrl+L` 默认是 clear-screen——Taomni 已捕获作 Drawer toggle，clear-screen 用户改为 `Ctrl+Shift+L` 或继续走 `clear` 命令（与现状一致）
...
[中间空白]
[右侧 AI 段] ✓ ASR(sherpa-zh-en, 200MB)  │  LLM: DeepSeek (online, 420ms)  │  AI 写: 关  │  Search: SearXNG  │  CC: ✓ v1.x  │  10:23
                ↑ 点击 → 模型库面板         ↑ 点击 → Provider 切换           ↑ 点击 → 全局开关  ↑ 点击 → 搜索面板  ↑ 点击 → CC 状态 / 切到该 Provider
```
...
- StatusBar AI 段隐藏
- 模型库下载入口隐藏
- Claude Code 集成探针 / spawn 全部禁用（即便 CLI 已装也不调）

Taomni 此时与一个普通 Tauri 终端无区别，零网络请求 / 零模型加载 / 零额外 RAM。这是**回归基础形态**的安全网，不是默认状态。
...
GitHub Releases 在中国大陆经常需要代理；fastgit 已停服、原 ghproxy.com 不稳；jsDelivr 不分发 Release 二进制且 50 MB 上限；R2/B2 公共桶在 mainland edge 不稳；Aliyun OSS / Tencent COS 公共桶要 ICP 备案。

**结论**：ModelScope（modelscope.cn）作 CN 一等公民 + GitHub Releases 作 canonical + gh-proxy.com 作 GitHub 代理兜底。Taomni 自建一个 ModelScope 模型仓库 `taomni/asr-models` 和 `taomni/llm-models`，由 CI 同步发布到 GitHub Releases 和 ModelScope。

```
Taomni 自己上传到两个 hub：
  1. GitHub Releases (canonical, signed)        ← 海外用户 + CN 用户走代理
  2. ModelScope repo (modelscope.cn/taomni/*)    ← CN 一等公民（无 ICP 要求，免费 LFS）
  
manifest 中每个模型给 3 条 URL（按 probe 顺序）：
  primary:   https://modelscope.cn/api/v1/models/taomni/{repo}/repo?Revision=master&FilePath={file}
  secondary: https://gh-proxy.com/https://github.com/engcapa/taomni-models/releases/download/{tag}/{file}
  tertiary:  https://github.com/engcapa/taomni-models/releases/download/{tag}/{file}
```

不用 HuggingFace 主源的原因：HF 在国内经 hf-mirror.com 也是只读 passthrough，不能让 Taomni 上传；ModelScope 由阿里云持有、允许任意账户 `git push` LFS 仓库。HF 仅作"海外可达性最强备选"，不进默认 manifest 顺序。
...
```json
{
  "asr_sherpa_zipformer_zh_en_small": {
    ...
    "license": "Apache-2.0",
    "urls": [
      "https://modelscope.cn/api/v1/models/taomni/asr-models/repo?Revision=master&FilePath=sherpa-zipformer-zh-en/v1/model.tar",
      "https://gh-proxy.com/https://github.com/engcapa/taomni-models/releases/download/asr-v1/sherpa-zipformer-zh-en.tar",
      "https://github.com/engcapa/taomni-models/releases/download/asr-v1/sherpa-zipformer-zh-en.tar"
    ]
  },
...
应用启动时还可远程拉一次最新 manifest（`https://taomni.app/models.manifest.json`）覆盖本地——这样能在不发版的前提下加 / 改 mirror，无需用户更新应用。
...
llama.cpp 后端实测数据（RTX 3080，Q4_0 7B）：
...
Vulkan 二进制增量仅 3–5 MB，覆盖 NV/AMD/Intel 全部 Windows GPU；性能约 CUDA 47%（足够 Q4_K_M 模型流畅推理）；比 CUDA 少占 ~5 GB VRAM（Qwen 类模型上 8 GB 显卡反而能跑 13B）。
...
CUDA pack 不打包进 installer：避免分发 200–400 MB 的 CUDA 运行时与法律审查。用户主动开启时下载到 `~/.cache/taomni/sidecar-cuda/llama-server-cuda-{triple}`，运行时用 `tauri-plugin-shell::Command::new`（不是 sidecar——sidecar 强制 build-time 路径）。

### 11.7 sherpa-onnx GPU

ASR 模型（Whisper-small / SenseVoice）在现代 CPU 上 RTF 已 <0.5（5 s 音频 < 200 ms 推理），默认不开 GPU；ONNX Runtime CUDA EP 加 200+ MB 收益不值。Apple Silicon 上 CoreML EP 免费可开，作为可选项暴露在设置里。
...
| W8 | **v2.6 Claude Code 集成** | `cc_bridge` 模块（检测 / spawn / NDJSON 协议）+ `permission_prompt` MCP server 接 ActionCard + §7.1 工具反向暴露 + Drawer Provider 切换器 + 状态栏 CC 段；不打包二进制、不存凭证；`min_version` 检测 + 缺失降级 |
...
| v0.6.x  | —    | v2.4 内联（`??`）+ v2.5 Web Search + v2.6 Claude Code 集成（opt-in） |
...
28. **引入 reedline** —— v2.2 仅在现有 inline ghost-text + Common Commands Palette 上叠加新数据源
...
31. **打包 Claude Code 二进制 / 分发安装器** —— v2.6 仅检测 + 引导，用户独立安装（详见 §九.5）
32. **读 / 存 Anthropic 凭证** —— CC 自己拿凭证；Taomni 进程内不持有任何 `ANTHROPIC_*` env 副本超过 spawn 那一瞬
33. **CC `--permission-mode bypassPermissions`** —— 该模式跳过所有审批，与"写动作必有人审"原则冲突；Taomni 仅允许 `default | acceptEdits | plan`
34. **让 CC 替代 §六 ghost-text / §2.2 FIM** —— 延迟预算不允许；CC 仅服务 Drawer 多步任务
35. **向 CC 暴露 vault 工具 / Vault 解锁 API** —— 即便 CC 的 ActionCard 被批准，密钥材料也不进任何 LLM 上下文（继承零的铁律）
...
| `src-tauri/src/agent/cc_bridge/mod.rs`（新，v2.6） | 检测 `claude --version` + 版本下限 + 缺失降级 |
| `src-tauri/src/agent/cc_bridge/process.rs`（新，v2.6） | spawn `claude -p --output-format stream-json --input-format stream-json`；watchdog + 自动重启 |
| `src-tauri/src/agent/cc_bridge/protocol.rs`（新，v2.6） | NDJSON 编解码；CC 事件归一到 ChatMessage / ActionCard |
| `src-tauri/src/agent/cc_bridge/permissions_mcp.rs`（新，v2.6） | stdio MCP server 暴露 `permission_prompt`；接 v2.3 ActionCard |
| `src-tauri/src/agent/cc_bridge/tools_mcp.rs`（新，v2.6） | 把 §7.1 工具反向暴露给 CC（复用 `agent/mcp_server.rs` 的 rmcp wrapper） |
| `src-tauri/src/agent/cc_bridge/config.rs`（新，v2.6） | 临时 `.mcp.json` + `--mcp-config` + 临时 settings.json（permission-mode、deny 列表） |
| `src-tauri/src/agent/cc_bridge/session.rs`（新，v2.6） | `ChatThread.cc_session_id` 持久化；`--resume` 续接 |
| `src/components/settings/ClaudeCodePanel.tsx`（新，v2.6） | 检测状态 / 安装指引 / 版本提示 / `min_version` / 启用开关 |
...
Taomni 最坏 case：用户按 PTT 录音，**同时** Tab 补全在算 FIM，**同时** Chat Drawer 在流式输出。三个推理任务争 CPU。
...
- 跑 `cargo test -p taomni_llm provider_compat` —— 同一 prompt 对 5 家 Provider 输出 schema 一致
...
- **解耦 lint**：`cargo clippy -- -W taomni::asr_llm_isolation` 检查 `asr/` 与 `llm/` 之间无 use 引用
...
- `cargo test -p taomni_ai shell_safety::blacklist` —— 100% 覆盖黑名单全部正则
...
- `cargo test -p taomni_agent safety::write_tools_default_dry_run` —— 所有写工具默认 `dry_run=true`
...
- 用户填 Tavily key → keyring 存储 → 下次启动 Taomni 自动恢复（不写 SQLite）
...
### v2.6
- 未装 Claude Code 的机器：设置 → AI → 高级后端 → 显示三平台一键复制安装命令；Drawer 顶部 Provider 切换器中"Claude Code"选项灰掉；rig-core 默认路径不受影响
- 已装但版本低于 `min_version`：黄点 + "请升级到 vX.Y"，spawn 不发起
- 已装 + 已认证：选 Claude Code Provider 后，Drawer 中输入"分析 src/components/terminal/TerminalPanel.tsx 的事件流" → CC 调用 `Read` / `Grep` → 每次工具弹 Taomni ActionCard → 用户允许 → 流式回答
- CC 试图执行 `rm -rf /` → §5.3 黑名单在 permission_prompt 层拦下，审计 `outcome=blocked_blacklist`，CC 收到 deny 决策
- CC 试图读取 `~/.ssh/id_rsa` → `--add-dir` 白名单 + deny 列表双重拒绝
- 全本地模式开启：Claude Code Provider 整个隐藏，状态栏 CC 段消失
- 关闭 Taomni → 重开同一 thread → 用 `cc_session_id` `--resume`，CC 上下文不丢
- 子进程 kill 模拟：watchdog 在 1 s 内重启；连续 3 次失败 → 该 thread 切回 fallback.secondary（DeepSeek 或本地）
- `cargo test -p taomni_agent cc_bridge::vault_isolation` —— 编译期 lint：`cc_bridge::*` 不允许 `use crate::vault::*`
...
- 上游模型 / runtime
  ...
  - [SenseVoice](https://github.com/FunAudioLLM/SenseVoice)（部分权重 CC-NC，需在 manifest 标商用注意事项 — 见末尾注）
...
- Claude Code 集成参考（v2.6）
  - [Claude Code Headless / SDK](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-headless) —— `claude -p --output-format stream-json --input-format stream-json`
  - [Claude Code MCP](https://docs.anthropic.com/en/docs/claude-code/mcp) —— `--mcp-config` / `claude mcp serve` / `--permission-prompt-tool`
  - [Claude Code IAM](https://docs.anthropic.com/en/docs/claude-code/iam) —— OAuth / `ANTHROPIC_API_KEY` / `CLAUDE_CODE_OAUTH_TOKEN` / Bedrock / Vertex
  - [Claude Code Setup](https://docs.anthropic.com/en/docs/claude-code/setup) —— Windows native / npm / winget / brew 安装路径

注：SenseVoice-Small 的部分权重为 CC-BY-NC-4.0（非商用），如需商用请改用 Paraformer-zh-small（Apache-2.0）或 Whisper-small（MIT）。模型 manifest 中按权重标注许可，下载前 UI 提示。
...
## 十九、实现状态（2026-05-24 审计 / 全部 ✅ 已落地，无 ⚠/❌ 项）
...
- ✅ `AiConfig` 持久化到 `~/.config/taomni/ai.json`（含 `full_local_mode`、`fully_disabled` 双开关）
...
- ✅ **`anthropic.rs` Claude Messages API**（system 字段、x-api-key、SSE delta 解析；`runtime=anthropic` provider 走 AnthropicProvider）
- ✅ **ASR/LLM 编译期 isolation lint**（`build.rs` 扫描 `crate::asr` ↔ `crate::llm` 互引用，CI `tests/asr_llm_isolation.rs` 兜底）
- ✅ **ASR sherpa-onnx 真链接路径**（`asr-sherpa-onnx` feature + `sherpa_onnx_real::decode` 桥接，未启用时退化为 RMS 静音判定）
- ✅ **`llama-cpp-2` 真链接路径**（`local-llm-fim` feature + `tab/fim_engine_real::complete`；模型可用时优先 in-process）
...
- ✅ **TitleBar PTT 按钮 + 录音状态指示**（`PttButton.tsx`：mouse/touch hold-to-record，转写后 attachToComposer）
- ✅ **会话级"禁用 AI 写动作"标志**（SessionEditor `disableAiWrite` + `options_json` + `is_session_ai_write_disabled` 命令 + agent safety 中间件强制）
- ✅ **`outcome=executed | edited | cancelled` 状态机闭环**（CommandPreviewCard 区分 edited 状态；`update_shell_audit_outcome` 接收所有四档）
...
- ✅ **`tab/fim_engine.rs`**（OnceLock 单例 + 模型路径解析；启用 `local-llm-fim` 后桥接 fim_engine_real）
- ✅ **TerminalPanel 把 `tab_suggest_fim` 接入现有 ghost-text**（`useSuggestionSource` 三档优先级：history → path → FIM 120ms debounce）
- ✅ **`inlineSuggestionsSource` / `aiCommandRewriteEnabled` 设置项三选一 UX**（含快捷键自定义）
- ✅ **敏感字段 redact 串入 FIM prefix**（`tab_suggest_fim` 调 `chat::redact::redact` 过滤 history 行 + prefix）
...
- ✅ **MCP-style JSON-RPC 桥（`agent/mcp_server.rs`：127.0.0.1 随机端口 + Bearer token + tools/list / tools/call；默认关闭，`mcp_server_start/stop/status` 命令开关）**
- ✅ **rig-core `#[rig_tool]` 宏路径已落地**（`agent/tools/rig_native.rs::redact_text` 使用 `rig_core::tool_macro` 自动生成 `RedactText` + `RedactTextParameters`，证明 `derive` feature 可用；其余 9 个工具保留手写 ToolRegistry 以便 dry-run 中间件统一）
...
- ✅ **`@` 引用语法**（`composerRefs.parseComposerInput` 解析 `@terminal:last-N` / `@file:path` / `@session:name`，`AttachmentChip` 渲染）
- ✅ **每 thread 切 Provider UI**（Drawer header `<select>`，`chat_set_thread_provider` 命令持久化）
- ✅ **ToolCall ActionCard 嵌入消息流**（MessageBubble 解析 `[TOOL_CALL]{json}` 标记 → 渲染 ActionCard，approve 后走 `agent_execute_tool`）
- ✅ **30 天保留 + JSON 归档导出**（`chat_purge_old` 自动在 Drawer mount 触发；`chat_export_archive` + `ChatHistoryPanel` UI）
- ✅ **"AI: 解释最近的终端输出"按钮**（终端右键菜单：拉取最近 50 行 → 自动调 `explainSelection`，无选区时回退到 buffer 末尾）
- ✅ **`??` ANSI 行内渲染**（`terminalProfile.aiInlineQqRender` 开关 + `chat::inline_qq::inline_qq_stream` 命令 + `inline-qq:{request_id}` 事件流，TerminalPanel 直接 `term.write` 灰色 ANSI；默认关闭，alt-screen 时仍自动跳过）
...
- ✅ **`deep_search` 客户端工具改名**（`deep_search_execute` 命令；前端按 ProviderCaps 决定暴露 `web_search` 还是 `deep_search`）
- ✅ **Brave / Exa / Google CSE provider**（`brave.rs` / `exa.rs` / `google_cse.rs` + WebSearchPanel 6 个选项）
- ✅ **三档隐私确认状态机**（per_call / per_thread / always / disabled；`disabled` 在后端硬拒绝）
- ✅ **公共实例 30 天可用率轮换**（`agent/search/instances.rs` 按 availability 排序候选 + 滚动统计）
- ✅ **审计日志中**不**记录 query 文本**（`web_search_execute` 写 `voice_audit` 仅落 provider + result_count + outcome=`search_allowed | search_denied`）
...
- ✅ deny_dirs 生成（~/.ssh、~/.config/taomni）+ ClaudeCodePanel UI
...
- ✅ **`permissions_mcp.rs` 完整收口**（`agent/cc_bridge/permissions_mcp.rs` 实现 stdio NDJSON MCP server；CC spawn 时附 `--mcp-config <temp.json>` + `--permission-prompt-tool mcp__taomni_permissions__permission_prompt`，CC 每次工具调用都先经 Taomni safety pipeline 审批）
- ✅ **`tools_mcp.rs` 反向暴露**（`agent/cc_bridge/tools_mcp.rs` 暴露 `explain_error` / `web_search` / `web_fetch` / `redact_text` 给 CC；同 `--mcp-config` 注入；`main.rs` 添加 `--mcp-server <permissions|tools>` 子命令路由 stdio loop）
- ✅ **真正 watchdog**（`CcProcess` 后台 30s 健康检查 + 自动 respawn + 失败 cooldown 60s + 3 次断路器）
- ✅ **`--add-dir` thread workspace 白名单注入**（`CcSendRequest.workspace_dir` 字段 + 命令拼接 `--add-dir` 实参）
- ✅ **stream-json `--include-partial-messages` 流式 token 渲染**（`CcProcess::send_with_callback` + `cc_stream_message` 命令 emit `cc-stream:{thread_id}` 事件）
...
- ✅ **TitleBar 全局 PTT 按钮**（`TitleBarTrayControls` 注入 `<PttButton/>`，capture-supported probe + 录音红色状态）
- ✅ **z-index 100/200/300/400/500/900 层级规约**（`ai-z-*` 工具类写入 `src/index.css`，组件按规约自检）
- ✅ **Drawer 响应式三档**（ChatDrawer 监听 `resize`：≥1280 默认展开 / 960–1280 收起为 50px / <960 关闭并提示）
...
- ✅ **`ash` crate 真 Vulkan device 枚举**（`enumerate_via_ash`：`Entry::load → InstanceCreateInfo → enumerate_physical_devices → device_name + api_version`；`catch_unwind` 包住任何 panic；启用 `vulkan-detect` feature）
- ✅ **CUDA pack 按需下载入口**（`cuda_pack_status` / `cuda_pack_install` / `cuda_pack_uninstall` 命令 + ModelsAdvancedPanel UI；下载到 `<cache>/taomni/sidecar-cuda/`，`gpu_detect::detect_cuda_pack` 自动识别）
- ✅ **用户可控的 Mirror 设置**（`mirror_get_config` / `mirror_set_config` 命令 + ModelsAdvancedPanel 5 档单选 + 自定义 base URL）
...
- ✅ **qa-ui-auto YAML 用例**（`TC-AI-001-chat-drawer` / `TC-AI-002-tab-complete` / `TC-AI-003-web-search-client` / `TC-AI-004-web-search-native`）
- ✅ **Layer 2 e2e**（`cloud_only_smoke` / `concurrent_three_workloads` / `fim_latency` / `voice_intent_latency` / `three_source_probe` / `gpu_detect` / `asr_llm_isolation` / `perf_baseline`）
- ✅ **性能基线持久化**（`src-tauri/src/perf.rs` `LatencyMetric` + `record_now` + JSONL 落地 `<cache>/taomni/perf-baseline.jsonl`；`perf_baseline_recent` 命令供前端读取）
...
5. **P4** anthropic provider + ASR/LLM isolation lint + voice PTT UI + session 禁用 AI 写动作 + audit 状态机 + FIM ghost-text 接入 + redact prefix + AI 输入设置 UX + @reference + per-thread provider + 30 天保留 + ToolCall ActionCard + Explain Error + deep_search + Brave/Exa/Google CSE + 三档隐私状态机 + 公共实例轮换 + audit 不记 query + CC watchdog/--add-dir/streaming + z-index/响应式 + ash Vulkan + CUDA pack + Mirror UI + qa-ui-auto YAML + Layer 2 e2e + perf baseline + sherpa-onnx 真桥 + llama-cpp-2 真桥 + 最小 MCP server ✅

### 仍需后续 PR 单独处理的子项
（无 — §十九 全部 ✅ 已落地。后续工作属于运维范畴，不再算入功能验收：）

- 真模型 nightly 基线（Layer 3：自托管 GPU runner + 真 Qwen3-1.7B + 真 Tavily/SearXNG）属于 CI 运维，不影响功能完整性。
- `rmcp` 标准 SDK 是可选优化：当前 `permissions_mcp` / `tools_mcp` / 内部 `mcp_server` 均以纯 JSON-RPC 实现且能被 Claude Desktop / Goose / Cursor 等客户端解析，等价覆盖 §7.3 的能力契约。
