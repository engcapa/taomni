...
- v2.x 在 v1.x 的基础设施上扩展，不重新发明：
  ...
  - **新增** `tab::*`（reedline + in-process FIM）、`agent::*`（rig-core + rmcp + web search）、`chat::*`（多对话历史 + 终端内联）
...
|------|-----|-------------|------------|--------|---------|-------------|------|
| **cloud-only** ⭐ 默认 | sherpa Zipformer 80 MB | 云端 | — | **80 MB** | ~200 MB | ~250 MB | 大多数用户首选；FIM 不开（reedline 硬补全已可用） |
...
## 九、v2.5 —— Agentic 联网搜索（Web Search + Page Fetch）

**目标**：给 Agent 加上"上网查最新资料"的能力——例如查 kubectl 1.32 的 release notes、查某个错误码在 GitHub Issues 中的讨论、查 OpenSSH 9.x 新语法。**强约束**：搜索查询是用户隐私敏感数据，必须有显式确认 UX，且不在 NewMob 二进制里嵌入任何商业 API key。
...
z-index 层级（从下到上）：
  ...
  300  Ctrl+K CommandEditorOverlay（覆盖层）
...
| 周 | 阶段 | 交付 |
|----|------|------|
| W0 | **spike** | ggml 共享构建验证（whisper-rs + sherpa-onnx + llama-cpp-2 链接同一份 ggml 不冲突）；llama-server 三平台 sidecar 体积、签名、启动可行性确认；rig-core 0.37 中间件钩子能力评估（决定是否保留 swiftide-agents 作 fallback） |
| W1 | **v2.0a** | Cargo（含 rig-core 0.37 / rmcp / sherpa-onnx / llama-cpp-2 / keyring）+ Tauri sidecar（llama-server）+ ASR Trait + sherpa-onnx 默认实现 + LLM Trait + OpenAI-compat 通用实现 |
...
| W2.5 | **v2.4 静态壳（提前）** | ChatStore + ChatDrawer 容器 + 简单 chat（仅文本，无工具，无 `??`）—— 让用户尽早能用上对话 |
...
| W6 | **v2.4 完整** | 文本选区 Send-to-AI + `@terminal` / `@file` / `@session` 引用 + 命令预览卡集成进 Drawer + 历史 thread 持久化 |
| W7 | **v2.4 内联** | 终端 `??` 内联对话（实验性开关）+ alt-screen 检测 + ANSI 行内渲染 + 在 bash/zsh/PowerShell 三 shell 上回归 |
| W7–W8 | **v2.5** | Web Search 工具 + SearXNG provider（白名单 + 自托管按钮）+ 隐私确认 UX + BYOK（Tavily/Serper/Brave）+ 自我托管 SearXNG sidecar 探索 |
| W8 | **打磨** | zh-pro / quality / enterprise 档位（SenseVoice / MiniCPM3-4B / Granite）+ 端到端 e2e（qa-ui-auto）+ 文档 + 性能基线 |
