...
export default function HBaseShellTab({ tabId, info, visible }: HBaseShellTabProps) {
  ...
  const [tables, setTables] = useState<HBaseTableInfo[]>([]);
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      setTables(await hbaseListTables(connectionSessionId));
    } catch (err) {
      setError(String(err));
    } finally {
  ...
  }, [info.host, info.namespace, info.port, info.restPath, info.ssl]);
  ...
  return (
    <div className="h-full w-full flex" style={{ background: "var(--taomni-bg)", color: "var(--taomni-text)" }}>
      <aside className="w-[260px] min-w-[220px] border-r flex flex-col" style={{ borderColor: "var(--taomni-divider)" }}>
        <div
          className="h-8 flex items-center gap-1.5 px-2 border-b text-[12px] font-semibold"
          style={{ borderColor: "var(--taomni-divider)", background: "var(--taomni-quick-bg)" }}
        >
        ...
        <div className="px-2 py-1.5 text-[11px] text-[var(--taomni-text-muted)] border-b break-words" style={{ borderColor: "var(--taomni-divider)" }}>
          {endpoint}
        </div>
        <div className="flex-1 min-h-0 overflow-auto taomni-scroll-y py-1">
          ...
          {connectionSessionId && tables.length === 0 && !tablesLoading && (
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <div className="h-8 flex items-center gap-2 px-2 border-b" style={{ borderColor: "var(--taomni-divider)", background: "var(--taomni-quick-bg)" }}>
          <button
          ...
          <button className="taomni-btn" type="button" onClick={() => quickCommand("help", true)} disabled={!connectionSessionId || running}>
            Help
          </button>
          <button className="taomni-btn" type="button" onClick={() => quickCommand("list", true)} disabled={!connectionSessionId || running}>
            List
          </button>
        ...
        <div className="h-[150px] border-b flex flex-col" style={{ borderColor: "var(--taomni-divider)" }}>
          <textarea
            ref={commandRef}
            data-testid="hbase-shell-command"
        ...
        {history.length > 0 && (
          <div className="h-[104px] border-t overflow-auto taomni-scroll-y" style={{ borderColor: "var(--taomni-divider)" }}>
            {history.map((entry) => (
              <button
