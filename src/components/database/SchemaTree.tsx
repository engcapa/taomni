...
export function SchemaTree({
  ...
  return (
    <div className="h-full flex flex-col" data-testid="schema-tree">
      ...
      >
        <Database className="w-3.5 h-3.5 text-[var(--moba-accent)]" />
                ...
                return (
                  <div key={tbl.name}>
                    <button
                      type="button"
                      className="moba-tree-row w-full text-left"
                      style={{ paddingLeft: 18, background: isSel ? "var(--moba-selected)" : undefined }}
                      onClick={() => {
                    ...
                    {st?.expanded && (
                      <div>
                        {st.loading && (
                          <div className="px-2 py-0.5 text-[var(--moba-text-muted)]" style={{ paddingLeft: 36 }}>
                            Loading…
                          </div>
                        )}
                        {st.columns?.map((col) => (
                          ...
                          >
                            {col.primaryKey ? (
                              ...
                              <Columns3 className="w-3 h-3 text-[var(--moba-text-muted)]" />
                            )}
                            <span className="flex-1 truncate">{col.name}</span>
                            <span className="text-[10px] text-[var(--moba-text-muted)] truncate max-w-[80px]">
                              {col.type}
                            </span>
                          </div>
                        ))}
                        {(st.indexes ?? []).length > 0 && (
                          <div
                            className="px-2 py-0.5 text-[10px] text-[var(--moba-text-muted)] flex items-center gap-1"
                        ...
                        {st.indexes?.map((idx) => (
                          ...
                          >
                            <ListTree className="w-3 h-3 text-[var(--moba-text-muted)]" />
