...
| `native_windows_ime_keys` | `{selector, layout?, keys, label?}` | Native Windows only. Activates the requested Windows keyboard layout for the packaged QA window, injects Win32 `SendInput` keys through the real IME, records foreground/layout facts, and restores the prior layout during teardown. It never fabricates composition events; testcase assertions own the postcondition. |
...
| `capture_workspace_restore_performance` | `{artifact_name, sample_id?, group?, wait_timeout_sec?, require_tab_count?, require_failed_count?}` | Native packaged app only. Reads the production restore observation after the testcase has exercised the real restore path, optionally polling until the production all-ready event or the supplied timeout, verifies target/read/settled counts and active/all-ready timestamps, and writes the raw event artifact. It does not drive restore or inject application state. |
