...
| 文件树 | tree/compact/flat 三种视图、名称过滤、新建/重命名/删除、Git 状态徽标、树字号缩放 | `CodeWorkspaceTab.tsx`（当前约 3.8k 行单文件） |
...
`CodeWorkspaceTab.tsx` 当前约 3.8k 行，继续堆功能不可维护。重构为：
...
| 里程碑 | 内容 | 规模 | 状态 |
|--------|------|------|------|
| **M0 前置重构** | 组件拆分 + codeWorkspaceStore + 命令系统骨架 + 底部 dock 容器（References 迁入） | M | 🔶 2/4，部分完成 |
...
- [ ] 组件全量拆分（FileTreePane / EditorGroup / 弹窗群 → §6.1 目录结构）+ `codeWorkspaceStore` — **⚠ 技术债：`CodeWorkspaceTab.tsx` 当前 3,775 行且持续增长，M3 分屏前必须补课**
