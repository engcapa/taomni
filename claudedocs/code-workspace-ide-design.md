...
>
> 日期：2026-07-11 · 版本：v2.3（实现状态复核，见 §8.1–8.2）· 状态：**实施中**（分支 `feat/code-workspace-ide`）
...
| 文件树 | tree/compact/flat 三种视图、名称过滤、新建/重命名/删除、Git 状态徽标、树字号缩放 | `CodeWorkspaceTab.tsx`（当前约 3.6k 行单文件，文件树视图壳已抽离） |
...
`CodeWorkspaceTab.tsx` 当前约 3.6k 行，继续堆功能不可维护。重构为：
...
| 里程碑 | 内容 | 规模 | 状态 |
|--------|------|------|------|
| **M0 前置重构** | 组件拆分 + codeWorkspaceStore + 命令系统骨架 + 底部 dock 容器（References 迁入） | M | 🔶 3/5，部分完成 |
...
- [x] `FileTreePane` 展示边界抽取（工具栏、视图/缩放控制、语言服务器面板）+ 组件测试 — 本次提交
- [ ] 剩余组件拆分（树数据/菜单控制器、EditorGroup、弹窗群 → §6.1 目录结构）+ `codeWorkspaceStore` — **⚠ 技术债：`CodeWorkspaceTab.tsx` 当前 3,590 行，M3 分屏前必须补课**
...
- [x] Go to File（双 Shift / Ctrl+Shift+N / Ctrl+P，camelCase 模糊匹配）— `972ad00`；当前 `SearchEverywhere` 仍是 Files-only 快选，不等同于完整六分组入口
...
- [x] 代码与自动化复核（2026-07-11）：`pnpm build` 通过；Code Workspace 定向 Vitest 13 个文件 / 68 项通过；LSP 定向 Rust 6 项、全文搜索定向 Rust 5 项通过
- [ ] **⚠ 真机验证欠账**：`65ac601` 起的功能均为测试级验证，需 `pnpm tauri dev` 人工过一遍（重点：补全/auto-import、签名帮助、Ctrl+F12、Ctrl+Shift+F、双 Shift、Ctrl+E、树右键）
- [ ] ⚠ 后端既有测试失败 8 例（2026-07-11 复核：717 通过 / 8 失败 / 11 ignored；config 剪贴板 URI ×4、terminal pushd ×1、workspace git 根检测 ×3），与本特性无关，合并前需处理或建立基线豁免
...
2. **补齐 M0 硬前提**：拆出 `FileTreePane` / `EditorGroup` / 弹窗群，引入按 `workspaceInstanceId` 分片的 `codeWorkspaceStore`；随后建立 `workspaceCommands.ts`，统一快捷键、右键菜单与 Search Everywhere Actions。
...
6. **补齐文件树交互**：完成剩余右键动作、键盘导航与拖拽；之后进入 M3 的分屏/tab/面包屑/终端/Run Tasks。
7. **处理合并门禁债务**：修复或显式豁免 8 个既有 Rust 失败用例，并把本轮真机冒烟结果补回本节。
