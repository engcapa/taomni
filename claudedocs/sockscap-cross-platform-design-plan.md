...
状态：Implementation in progress — Revision 6
初始日期：2026-07-18
最近更新：2026-07-19
本版范围：在 Revision 5 已冻结的共享数据面与 Windows 捕获选型之上，补齐生产发布所需的源码/manifest 契约收敛、最低支持矩阵、Windows 用户态签名账号，以及更新、安全响应和现场支持 Gate；本轮进一步落地 replaceable packet-stack supervisor/first-terminal latch、packet-device 防串线身份与严格 IPv6 admission、Linux mutation-before-WAL/PID incarnation 校验、双向 TUN pump、ready-before-activate、取消时所有权保留、自动 terminal-fault withdrawal 和 cleaned-generation tombstone 等 **source contract**。这些代码仍没有具体受控 TCP/UDP/reassembly/Virtual-DNS provider、Linux 产品组合 factory/`CaptureAdapter`/coordinator 接线或真实平台证据；S1、三平台接通、打包/native evidence 和生产发布 Gate 均未整体通过。
...
| 共享数据面 | `FlowDescriptor`、immutable/bounded `ProfileSelector`、bounded decoded `FlowIngress`、L3 `PacketIngress` queues、strict bounded IPv6 extension parser、pinned `IpStackConfig`/bounded `PacketFlowRegistry`、snapshot-bound `FlowEngine`、supervisor-owned `FlowRuntime`；`PacketStackSupervisor` 固定 provider source pin/capability/generation/revision/platform identity，以同一绝对 deadline 等待 one-shot ready，保持 first terminal event，并只允许一次 ingress handoff，shutdown timeout 保留 join handle；packet queues 以 opaque/private `source_id` 拒绝 cross-wire；flow registry 用显式 TCP/UDP 计数 | 这些均为 source/memory contract。handoff 返回 `Arc`，唯一 `FlowRuntime` 消费仍是产品 factory invariant；fragment 只进入 reassembly admission。仍缺具体受控 TCP/UDP/reassembly/Virtual-DNS provider、产品组合层、真实 adapter/orchestrator、IPv6 scoped DIRECT 真实发送及 native/performance/24h/7d Gate |
| Linux 特权与客户端 primitive | cgroup v2 + nftables + fwmark/TUN 事务、root helper、SO_PEERCRED、可执行文件 pin、HMAC；helper 在首次 mutation 前 fsync 完整 restore receipt，membership delta 也先 WAL；Linux PID 使用内核 start tick 并在 mutation 前后重验；fixed-path client/session、generation-only recovery、exact TUN source、双向 pump、ready-before-activate、取消安全 owner、cleaned tombstone，以及 source-level helper-first terminal-fault withdrawal | 尚无具体完整 stack provider、Linux 组合 factory、产品 `LinuxCaptureAdapter`/coordinator/orchestrator reconciliation、可信 tuple→profile side channel、polkit/package artifact 或真实 root/native fault smoke；source lifecycle 不是产品接通，因此 capability 仍全部为 false |
...
| 生产发布运维 | updater 基础链路和签名 receipt 结构 | updater 私钥托管/轮换/吊销、分阶段发布/停止放量/回滚、持续 SBOM/CVE、威胁模型、脱敏支持包与符号保留 |
...
- Linux 的 cleanup 思路已扩展为 Taomni 独立 helper、mutation-before-WAL root receipt、cleaned-generation tombstone、recovery journal 和下次启动修复；fixed-path client/session、双向 TUN pump、lifecycle ownership 与 source terminal-fault withdrawal 已有源码合同，但仍需产品组合 factory/`LinuxCaptureAdapter`/coordinator 接线和真实 kill/断电平台证据。
...
- 当前已落地并硬化 bounded decoded-stream 合同：`FlowDescriptor` 对 generation、平台、五元组、PID/start token、app identity、capture intent 与 profile binding fail closed；只有明确的 global capture 才能使用 global fallback，app/PID 归因缺失或未匹配直接拒绝，`include_children` 只接受可信 revision queue。`ProfileSelector` 限制 profile/selector 数量；`FlowEngine` 绑定 config revision 与完整 profile SHA-256；`FlowRuntime` 由不可直接 abort 的 supervisor 持有任务和 egress owner，以有界 close executor、exactly-once close state、同一绝对 shutdown deadline 和一致统计完成内存 TCP half-close relay。它尚未接入任何真实平台或 orchestrator。
- 当前新增的 replaceable `PacketStackSupervisor` 只定义 provider 边界，不内置 production stack：descriptor/driver/ready 必须匹配精确 provider pin、generation、config revision、platform 与显式 IPv4/IPv6/TCP/UDP/reassembly/Virtual-DNS capability；native packet queues 还必须来自同一个 opaque/private `source_id`，防止同 generation/platform 的 ingress/egress 被交叉拼接。provider build 与 ready 共用一个绝对 startup deadline，decoded-flow receiver 只允许一个 `FlowRuntime` owner 取走；shutdown timeout 不丢弃 join handle，可在同一 owner 上重试。
- startup future 被取消或 ready 前 owner 被丢弃时，supervisor 会取消 driver 并把 join handle 交给有界 emergency reaper；这只是最后的资源上限措施，**不构成 clean shutdown、helper 撤销或发布 cleanup 证明**。产品组合层仍必须显式等待 stack、`FlowRuntime` 与 native pump 停止，并将 Active 后的任一 fault 自动升级为 capture stop/recovery。
- `PacketFlowRegistry` 维护显式 active TCP/UDP 计数，避免每次 admission 为统计 transport 数量而线性扫描整个 flow table；这不把 `HashMap` 的全部操作宣称为严格 worst-case O(1)，也不替代容量、性能和长稳证据。
...
- `ipstack` 1.0.1（tag `v.1.0.1`，commit `a343ea8c696e761acce8dbcd6687c862ecd8aacd`）源码审计发现多条 unbounded channel 路径，当前不得进入 production dependency。先做 isolated spike，再决定受控 bounded fork/upstream patch 或替代 adapter，并关闭 UDP/lifecycle/resource Gate。
...
S0 已冻结：Taomni FlowRuntime/IP-stack adapter + Windows Wintun/WinDivert
  ...
  └─> S1 实现 Packet/Stream 双入口数据面与身份合同
       └─> S2 将真实 adapter/runtime 接入 orchestrator + recovery
            ├─> L1 Linux client/TUN/pump lifecycle source → concrete stack composition + product adapter/package
...
| S1-STACK-SUPERVISOR（PARTIAL：source contract） | single-use `PacketStackSupervisor` 校验精确 provider pin/capabilities/generation/revision/platform/queue identity；build + one-shot ready 共用绝对 deadline；first-terminal-event latch 保持真实根因；decoded ingress 只允许一次 handoff；shutdown timeout 保留 join handle。返回 receiver 是 `Arc`，唯一消费是产品 factory invariant；Drop/start-cancel emergency reaper 不算 cleanup proof | `flow/packet_stack.rs`、`capture/packet_device.rs`、`flow/ip_stack.rs` | 仍须实现并固定具体受控 TCP/UDP/reassembly/Virtual-DNS provider；产品 factory 必须唯一消费 ingress、启动 `FlowRuntime`、聚合 native/stack/runtime health，并接入平台 capture withdrawal；native/performance/24h/7d 前不得 PASS |
| S1-LINUX-CAPTURE（PARTIAL：source lifecycle） | helper 先无副作用规划、fsync 完整 root restore receipt，再做 cgroup/TUN/network mutation；membership delta 也 write-ahead。typed client/session、TUN owner/name/fdinfo、双 pump ready 已实现；lifecycle 以 generation/revision/runtime PID+kernel start token/queue identity 校验数据面并串行 mutation；terminal await 保留 owner。cleaned tombstone 先 durable publish 再删 receipt。source fault monitor 对 terminal/heartbeat ambiguity 执行 helper-first withdrawal并停止本地 plane，保留根因 | `capture/linux_client.rs`、`capture/linux_helper.rs`、`capture/linux_process.rs`、`capture/linux_tun.rs`、`capture/linux_adapter.rs`、`capture/coordinator.rs` | operation mutex/root receipt 不能替代 durable product coordinator journal；numeric `cgroup.procs` 不能把 process incarnation 与 move 做成一个原子内核操作，仍须 pidfd/kernel API 评估和 PID-reuse stress lab。另缺具体 stack/factory、产品 adapter/coordinator/orchestrator reconciliation、tuple side channel、polkit/package/root fault evidence；能力保持 false |
| S1-TCP（PARTIAL） | `FlowRuntime` 已实现 supervisor-owned handle、有界 admission/close、duplicate/overload/stale rejection、snapshot/owner binding、EOF half-close、waiter-abort cleanup、panic/hang close-once、单一绝对 shutdown deadline、一致终态统计与 partial-byte relay accounting；`PacketFlowRegistry` 固定 provider pin、TCP/UDP 上限、tuple family/protocol 检查和不可变 capture identity，并以显式 active TCP/UDP counters 去除 admission 的线性 transport-count scan | `flow/runtime.rs`、`flow/engine.rs`、`flow/ip_stack.rs`、现有 connectors | 已有 memory slice；完整 PacketIngress→TCP/UDP state、RST/fragment reassembly/Virtual DNS/IPv6 scope 发送、真实 connector/native adapter、性能与 24h/7d 仍未完成 |
...
| S2-RUNTIME | 实现 `PlatformAdapterFactory`/runtime handle，将真实 adapter、packet-stack、`FlowRuntime`、heartbeat/fault monitor 和 coordinator 组合到 `SockscapEngine::start/stop/recover` | `capture/runtime.rs`、`orchestrator.rs`、`commands.rs` | start 顺序固定为 upstream/probe ready → helper/provider prepare（尚未重定向）→ native pump + stack + `FlowRuntime` 组合 ready/healthy → capture activate → 组合健康复检/live self-test → Active；Active 后任一 component fault 自动 stop/recover；任一步失败完整回滚 |
...
| S2-RECOVERY | 让 recovery journal 同时记录平台 artifact 与 data-plane generation；恢复先撤销系统规则，再关闭本地 gateway/runtime；Linux cleaned tombstone 可证明特定 generation 已完成幂等清理，且 response 丢失后仍可重试 audit | coordinator/store/platform adapter | main/helper/provider 在 prepare、activate、Active、stop 的每个注入点死亡均能在本次或下次启动恢复；tombstone 必须经真实 root crash/断电测试。既无 pending receipt 又无匹配 cleaned tombstone 时拒绝猜测性清理并显示人工恢复步骤 |

共享数据面必须先用**内存 PacketDevice/StreamIngress**完成确定性测试，再接触真实系统路由。这样平台测试失败时可以区分“捕获/身份问题”和“TCP/IP/egress 问题”。

当前只完成 bounded decoded StreamIngress、strict L3/IPv6 PacketIngress contract、IP-stack admission/supervisor source boundary、Linux helper WAL/PID/client/TUN、双向 pump 和 lifecycle/fault-withdrawal source contract；尚未接入具体完整 stack provider、产品组合 factory 或 `CaptureAdapter`。不要把 ingress 10 + packet-device 12 + selector 11 +
runtime 12 + engine 13 + IP-stack admission 10 + packet-stack supervisor 17 + Linux helper 10 + Linux process 12 + Linux TUN 4 + Linux client 12 + Linux adapter lifecycle 24 个 focused tests，或完整 Sockscap 软件测试 PASS，解释成 PacketIngress/IP-stack、UDP、
平台 adapter、orchestrator、native smoke、性能或 24h 已通过。

#### 13.0.4 Linux 接通工作包

当前 Linux 已完成 privileged mutation/server、mutation-before-WAL/PID-incarnation checks，以及 fixed-path typed helper client/session、generation-only recovery、cleaned tombstone、exact TUN L3 source、双向 pump ready-before-activate、cancellation-safe ownership 与 source fault withdrawal contracts；以下产品组合、接通、打包和真实证据仍为剩余工作，capability 继续保持 fail-closed：

| ID | 代码/打包任务 | 验证出口 |
|---|---|---|
| L1-CLIENT | 将已实现的 typed `LinuxHelperClient`/actor session、`LinuxCaptureLifecycle` 与 `RealLinuxHelperTransport` 封装为产品 `LinuxCaptureAdapter`：只允许 `/usr/bin/pkexec` 启动 `/usr/libexec/taomni/sockscap-helper`，读取 `/etc/taomni/sockscap-helper-policy.json`，连接 `/run/taomni` socket，验证 root peer SHA-256，完成 HMAC bootstrap 与 prepare/activate/update/heartbeat/stop/recover/recover_generation | 非 root 调用、错误 UID/PID/hash、符号链接/非 allowlist policy、旧 generation、重放/篡改全部拒绝；client cancellation 不复用半帧 session；fake-helper 合同不是产品接线，真实 polkit、artifact/package receipt、coordinator reconciliation 与 fault monitor 证据全部通过后才完成 |
...
| L1-TUN | 已实现 `LinuxGlobalTunReader` 和 native capture/reinject 双向 pump：TUN 使用 `TUNSETIFF + IFF_TUN + IFF_NO_PI`、`TUNGETOWNER`、CLOEXEC/nonblocking 与有界 MTU/L3 validation；activation 前以 runtime/queue identity 和双 pump ready 证明 owner 已接管。下一步实现 Linux factory，使用 supervisor one-shot handoff 将 packet stack 交给唯一 `FlowRuntime`，聚合三者 health | 当前只有 source/fake contract；仍需真实 IPv4/IPv6 echo/HTTP、MTU/fragment/checksum/RST/并发/runtime death、完整 TCP/UDP/reassembly/Virtual-DNS provider 与 product fault reconciliation；helper 不能被误认作 runtime |
| L1-CLEANUP | 将已实现的 mutation-before-WAL、operation serialization、取消时 owner retention、helper-first terminal/heartbeat fault withdrawal 和 cleaned tombstone 接入 coordinator/installed helper。tombstone 必须与 generation/UID/artifact identity 完全匹配，先 publish+fsync 再删 receipt；生产 GC 只能由 durable coordinator journal/rollback low-water mark 驱动 | prepare/activate/Active/stop 每个 crash/cancel 点，helper response 丢失、receipt+tombstone 双记录、foreign/conflicting tombstone、GC 边界、主进程/helper/data-plane kill 和断电重启均有 root VM 证据；response-loss retry fail-closed；product coordinator 对 source monitor 的 cleanup/result 必须一致落 journal |
| L1-SCOPE | 完成 global、application group、running PID 与 include-children；保留 PID/platform-native incarnation token 和原 cgroup，Linux 使用 kernel start tick 并紧邻 mutation 重验，处理退出/PID 重用；为 native capture 建立可信 tuple→profile side channel；plain global TUN 不提供伪造 PID | 选中/未选中进程同时发流；未选流不进入数据面；global + 两个 app profile 分别使用不同 egress；停止后只恢复仍属于该 generation 的精确 process incarnation；专门 PID reuse race lab 通过；side channel 缺失时 app/PID capability 保持 false |
...
| L1-PACKAGE | 冻结支持的 distro/kernel/systemd/cgroup/nft/iproute2/resolved/NetworkManager 矩阵；为 deb/rpm 安装 helper、policy、polkit action 和卸载恢复；决定 AppImage 是否只做 UI/非 capture 或配套 installer；未安装完整 artifact 不得把 source client/TUN primitive 当作能力 | 包签名/来源验证、root owner/mode/hash、固定 helper/policy 路径、安装升级回滚卸载；卸载后无 `/run/taomni`、nft table、ip rule/route、TUN、cgroup 或 policy 残留；在 package/native/root smoke receipt 之前 `capture_implemented` 与所有 `can_start_*` 仍为 false |
...
| ID | 代码/打包任务 | 验证出口 |
|---|---|---|
| W0-SUPPORT-MATRIX（Windows 范围已冻结；macOS/Linux 待冻结） | 第一版生产 Windows Sockscap Beta 只支持 `x86_64`。官方 WinDivert + 不自建驱动路线不发布 ARM64；probe 稳定 fail-closed。同时冻结 macOS 最低版本/Apple Silicon/Intel 与 Linux distro/kernel/systemd/cgroup/nft/iproute2/resolver/NetworkManager 范围；macOS overlay 的 `11.0` 只是 provisional build floor | Windows x86_64 manifest/installer/artifact/lab 一致；ARM64 fail-closed contract 通过。macOS/Linux 由带证据的矩阵修订关闭，未列入范围的系统不生成 release PASS |
...
| ID | 生产任务 | 验证出口 |
|---|---|---|
| P0-RELEASE-OPS（OPEN） | 指定 updater minisign 私钥 owner，采用受控 CI/HSM 或等价不可导出方案，建立离线恢复副本、访问审计、轮换/吊销 runbook；定义 app/helper/provider/driver 精确 pin/ABI、protocol/schema、journal/tombstone 与 ready-handshake 兼容矩阵；实现分阶段发布、停止放量、签名 rollback，并保护 lab/CI evidence producer、签发可验证 provenance/attestation | 隔离 release 演练完成正常升级、Active clean stop 后升级、暂停放量、坏版本回滚、私钥轮换/吊销恢复；旧组件拒绝不兼容组合，ready/health 失败不激活且系统 artifact diff 为零；manifest/artifact/typed receipts 与原始证据由签名 attestation 绑定同一不可变 candidate |
...
当前状态：**BLOCKED（选型已冻结，局部软件 slice/source contract 已实现，发布证据未完成）**。策略、生命周期、source release gate、Linux privileged transaction/helper 认证底座，以及 bounded decoded StreamIngress/identity/FlowRuntime、strict IPv6 L3 admission、replaceable packet-stack supervisor、packet `source_id` 防串线、Linux mutation-before-WAL/PID checks、双向 pump/组合健康/取消安全 ownership、source terminal-fault withdrawal 与 cleaned tombstone 已落地。Windows source contract 固定为 Wintun global + WinDivert app/PID。具体 production TCP/UDP/reassembly/Virtual-DNS provider、Linux 组合 factory/产品 adapter/coordinator、三平台真实 adapter/vertical slice、配置后的签名/entitlement policy、package/native 和平台恢复证据仍不存在；因此所有 capture capability 继续为 false。
...
- 继续落地 `S0-DATA-ADR`：保留已完成的 bounded StreamIngress/FlowRuntime 与 L3 PacketIngress/admission slice；对 ipstack 1.0.1 只做 isolated spike，先决定 bounded fork/upstream patch 或替代 adapter，再实现完整 PacketIngress/UDP bridge，并证明 Virtual DNS、IPv4/IPv6、取消、统计、fuzz、性能与 24h 资源边界。
...
- Linux：在已落地 cgroup v2 + nft socket cgroup + fwmark/TUN、helper WAL/PID、fixed-path client/TUN、双向 pump 与 source fault monitor 上，补具体受控 stack、stack + `FlowRuntime` factory、产品 `LinuxCaptureAdapter`/coordinator reconciliation、可信 tuple→profile side channel、polkit/package 与 root/native lab；验证 running PID race、global + 多 app profiles 与不支持环境降级，未取得证据前 capability 保持 false。
...
当前状态：**PARTIAL**。DIRECT、SOCKS5 TCP、HTTP CONNECT、共享 SSH `direct-tcpip`、严格 host-key、取消/统计边界和 packet-stack supervisor source boundary 已经落地；尚缺具体 production TCP/UDP/reassembly/Virtual-DNS provider、产品组合/adapter、DNS/UDP 闭环和三类真实服务器完整矩阵。
...
当前状态：**PASS（既有软件范围）**。SQLite/WAL、recovery journal、IPC/Stub、helper heartbeat、mutation-before-WAL root receipt、cleaned tombstone、取消时 lifecycle owner retention 与 source terminal-fault withdrawal 已落地；但产品 coordinator 接线/journal reconciliation 以及已安装平台 artifact 的恢复证据仍归入 Phase 5–8，不能由本阶段 PASS 代替。
...
当前状态：**PARTIAL（source primitives/lifecycle 已落地，产品接通仍 BLOCKED）**。真实 cgroup-v2/nft/fwmark/TUN transaction、root helper authentication、两阶段激活、mutation-before-WAL receipt、kernel start-tick PID revalidation、fixed-path typed client/session、generation-only recovery、exact TUN source、双 pump ready、组合 identity/health、mutation serialization、取消安全 owner、source terminal/heartbeat fault withdrawal 与 response-loss cleaned tombstone 已落地。仍缺具体完整 stack、stack + `FlowRuntime` factory、产品 `LinuxCaptureAdapter`/coordinator/orchestrator reconciliation、tuple→profile side channel、polkit/package、真实 root capture/fault smoke，因此四个 capability 位全部保持 false。

- 完成 13.0.4 的 `L1-CLIENT` 至 `L1-PACKAGE`：实现具体受控 stack provider与 Linux 组合 factory，把现有 client/TUN/pump/lifecycle source contracts 封装成受安装策略约束的产品 `LinuxCaptureAdapter`，接入 coordinator/recovery 与 active fault monitor，并取得 package/native/root evidence。
...
当前状态：**PARTIAL**。native tray/guarded exit 代码、recovery UI、Linux native window smoke、fixed core/per-platform 24h verifier 与 100 synthetic lifecycle cycles 已落地；真实托盘/system smoke、actual capture、24h、stable 7-day receipt/verifier、性能/泄漏和三平台 package evidence 尚缺。
...
| Stable staged 长稳 | 三平台 stable 候选在冻结支持矩阵中分阶段连续运行 ≥7 天；使用新增的非 synthetic receipt/schema/verifier，固定候选 commit/artifact/host，未解释 component exit、capture residue、DNS/IPv4/IPv6/UDP leak、资源越界均为 0 |
...
- 已通过：最终共享源码 Sockscap Rust 295/295（ingress 10、packet-device 12、selector 11、runtime 12、engine 13、IP-stack 10、packet-stack 17；Linux helper 10、process 12、TUN 4、typed client 12、adapter lifecycle 24），repository lib 1,174 total：1,163 passed、0 failed、11 ignored；Python Gate 10/10；macOS host/Bash 3.2 syntax+disabled-template lint；PowerShell 7.2.24 AST+disabled-template lint；frontend 67/67、TypeScript/Vite build、catalog/lint、Linux native window smoke、10,000-rule quick 与 100 synthetic cycles。Linux WAL/PID/client/TUN/adapter/supervisor 只证明 source contracts，不是产品 capture evidence。
- 仅执行证明：约 3.2 秒短 soak；它不能充当 24h receipt。
- 尚未实现或运行：具体 production IP-stack TCP/UDP/reassembly/Virtual DNS provider/bridge、Linux stack + `FlowRuntime` factory 与产品 `CaptureAdapter`/coordinator/orchestrator reconciliation、Windows/macOS 真实 adapter；Linux polkit/package/root/native evidence；typed real-capture producer；protected signed lab attestation；三平台真实 capture/egress/leak/performance/100-cycle/24h/7d、tray/system smoke、签名 package 安装/升级/卸载矩阵。Linux source fault withdrawal/WAL/pump/supervisor/tombstone 不能代替上述证据。缺口关闭前四个 capability 位不得改变。
...
| 数据面 Active 后静默死亡 | UI 仍显示 Active，而捕获可能黑洞、泄漏或停留在旧规则 | Linux source lifecycle 已订阅 terminal/heartbeat ambiguity 并执行 helper-first withdrawal，再停止本地 owners；仍须由产品 adapter/coordinator/orchestrator 对接 journal/status，并在真实 privileged lab 证明。Windows/macOS 也必须实现同等路径 |
...
31. 数据面 readiness 与 cleanup：provider ready 只表示精确 pin/identity/capability 的 event loop 接管了有界输入，不能单独解锁 capture capability。平台只有在 native pump、packet stack 与唯一 `FlowRuntime` 组合 ready/healthy 后才可 activate；Active 后任一组件 fault 必须自动撤销 capture。startup/drop 的 emergency reaper 不计入 clean-shutdown 或发布恢复证据。
