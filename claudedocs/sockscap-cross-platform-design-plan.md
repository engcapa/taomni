...
状态：Implementation in progress — Revision 4
初始日期：2026-07-18
最近更新：2026-07-19
本版范围：记录已落地底座，并定义 Windows、macOS、Linux 真实接通所需的代码、外部输入、验证矩阵与发布证据。
...
| Windows | 禁用模板、Authenticode/SignTool/driver package fail-closed verifier | Wintun/global 与 WinDivert 或 WFP app/PID 实现；最终 ADR；真实签名 helper/provider/driver 和安装包 |
...
| ID | 代码工作 | 主要落点 | 验收条件 |
|---|---|---|---|
| S0-DATA-ADR | 对 tun2proxy 与自有受控 IP stack 做限时 spike；固定版本、许可证/SBOM、per-flow selector、IPv4/IPv6、Virtual DNS、UDP、取消、统计 hook 和维护策略 | 更新 `sockscap-phase0-adr.md`；若采用第三方则固定 `Cargo.lock` 与 NOTICE/SBOM | 选择必须支持按 flow 调用 Taomni `PolicyEngine` 和四种 egress；不能只接受一个固定 proxy URL；没有结论不得把候选依赖标为生产数据面 |
| S0-WIN-ADR | 在真实 Windows + WDK 上比较 WinDivert 与 WFP ALE：身份 race、回注、双捕获、IPv6、许可证、签名、EDR/VPN、安装/卸载 | ADR amendment + 可重复 spike 工具/报告 | 明确选择、最低 Windows 版本、x64/arm64 范围、artifact/package 结构；`unselected` 从 Windows release manifest 中消失 |
| S1-INGRESS | 增加统一 `FlowIngress`，至少包含 `PacketIngress`（Linux TUN/Wintun）与 `StreamIngress`（macOS NE/WFP redirect）；统一生成 `FlowContext` | 建议新增 `sockscap/capture/packet_device.rs`、`flow/ingress.rs`、`flow/runtime.rs` | 同一 matcher/connector 测试可分别由内存 L3 packet 和已解码 TCP stream 驱动；无平台规则复制到 provider/helper |
...
| S1-GATEWAY | 为 macOS provider/WFP redirect 提供 authenticated loopback data gateway；控制面与数据面分离，协议带 version/generation/flow id/长度上限 | 建议新增 `capture/gateway_protocol.rs` 与本地 listener | 只监听 loopback/受控 IPC；peer identity、消息大小、flow 并发、超时、重复 flow id、旧 generation、主进程消失均 fail closed/fail open 符合策略；无 secret 进入 argv/log |
...
Windows 代码必须在 S0-WIN-ADR 选择后收敛，不能同时把两个候选都当生产路径：
...
| W1-SELECTED | 若选 WinDivert：实现 SOCKET/FLOW tuple→PID/start token cache、NETWORK capture/reinject、动态 filter 和 race 处理；若选 WFP：实现 ALE classify/redirect、callout/filter lifecycle 与 StreamIngress bridge | app/PID/children/new-connections；PID 重用；TCP simultaneous open/close；IPv6；高并发；未选 flow 不被延迟；global + selected 不双捕获、不丢 identity |
...
| W1-BYPASS | 在捕获层排除 Taomni/helper/service、Wintun/WinDivert/WFP 自身流量和所有动态 upstream endpoints；验证 reinjection 不再次命中 | 连接计数无递归增长；Defender/EDR/VPN 同时启用时无 packet loop；本机/LAN policy 与设计一致 |
| W1-INSTALL | 生成 MSI/NSIS 所需 service/driver/INF/CAT/DLL 安装与回滚；所有应用/helper 用 timestamped Authenticode；kernel package 使用 Microsoft 接受的生产签名路径 | Secure Boot 开启的干净 Windows；`signtool /pa` 与 `/kp`、INF↔CAT、publisher、timestamp、hash 全通过；不接受 test signing 或禁用 Secure Boot 的证据 |
...
Windows 功能实验室至少覆盖：最终支持的 Windows 版本、x64/arm64 范围、Secure Boot、Defender、企业 EDR 样本、常见 VPN、Wi-Fi/有线、睡眠/唤醒。WinDivert 若被选择，还必须完成 LGPL/GPL 分发合规和随包 LICENSE/NOTICE；自有 WFP driver 则必须完成 WDK/HLK 或批准的 Microsoft signing 流程。
...
| M3 — Windows Beta | 选定 provider、Wintun/selected adapter、signed service/driver、installer | Windows 全矩阵、签名、EDR/VPN、100-cycle、24h 和安装卸载清理通过 |
...
正式发布顺序仍遵从已确认决策：Windows 可先 Beta，三平台全部通过后才能称“跨平台稳定”。工程上 Linux 是当前最快的数据面反馈通道；这不改变 Windows Beta 的发布口径，也不允许跳过 Windows ADR。

### Phase 0：三平台能力与许可证 Gate

当前状态：**BLOCKED**。策略、生命周期、source release gate、Linux privileged transaction 和 helper 认证底座已经落地；但共享数据面/Windows provider 仍未冻结，三平台真实 vertical slice、真实签名/entitlement 和平台恢复证据均不存在。详细缺口见 13.0 的 `S0-*`、`S1-*` 与平台工作包。
...
- 完成 `S0-DATA-ADR`：tun2proxy 或替代核心必须证明 per-flow route hook、四种 action/egress、Virtual DNS、IPv4/IPv6、UDP 降级、取消、统计 hook 与多 profile 身份不会丢失。
- 完成 `S0-WIN-ADR`：在真实 Windows/WDK 环境比较 WinDivert SOCKET/FLOW/NETWORK 与 WFP ALE redirect；验证应用/PID、回注正确性、本机 loop bypass、许可证、签名和卸载恢复后只保留一个生产路径。
...
退出标准：冻结 `S0-DATA-ADR` 与 `S0-WIN-ADR`；三平台至少完成 TCP 到本地 echo/HTTP server 的 global + selected app 垂直切片；SOCKS5、HTTP CONNECT、SSH Jump 三种 egress 都通过可重复测试；停止或强制终止主进程/privileged component 后网络恢复；没有未解释的 DNS/IPv6/UDP 泄漏。任一平台失败则回到范围或架构评审，不得解锁对应平台 capability。
...
当前状态：**BLOCKED**。只有 fail-closed manifest、Authenticode/driver-package verifier 与候选 provider contract；没有真实 adapter、service/driver 或签名安装包。

- 先完成 `S0-WIN-ADR`，再按 13.0.5 的 `W1-GLOBAL` 至 `W1-UPDATE` 收敛 Wintun + 单一 app/PID provider。
- 将应用/PID/子进程/new-connections 身份接到共享 FlowRuntime；若采用 WinDivert，关闭 ownership race、动态五元组 filter、packet reinjection 与许可证问题；若采用 WFP，则关闭 ALE/callout/driver 生命周期与 WDK 签名问题。
- 交付 timestamped Authenticode app/helper/service 和 Microsoft 接受的生产签名 driver package；完成 Secure Boot、管理员提示、EDR/VPN 与安装/升级/回滚/卸载验证。

退出标准：全局、程序组、运行 PID、三种 egress、GFWList、托盘、Dashboard 全链路；同机 artifact gate 与 13.0.7 全矩阵通过；重启、崩溃和更新不遗留 Wintun/WinDivert/WFP/service/route/DNS 状态。只有此时 Windows Beta 可发布。
...
- [Microsoft Hardware Dev Center driver signing offerings](https://learn.microsoft.com/en-us/windows-hardware/drivers/dashboard/driver-signing-offerings)
