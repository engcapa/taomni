...
状态：Implementation in progress — Revision 8
初始日期：2026-07-18
最近更新：2026-07-20
本版范围：在 Revision 7 的共享 composition、Linux generation owner 与 DEB/RPM Gate 上，统一为公开且取消安全的 `ProductDataPlaneSupervisor` generation registry，拆分“运行根因诊断”与“当前 cleanup proof”，让所有 startup/stop/join timeout 都保留可重试 owner；Linux bridge 复用同一 supervisor。capture runtime 的 install/update/heartbeat/stop/reconcile 已改为 detached transaction；同一 `SockscapStore` 派生的所有 runtime owner 共享 store-scoped mutex，调用者取消不会释放 operation ownership，也不能通过另建 runtime 绕过串行化。recovery cleanup 前以 expected-generation CAS/revalidation 拒绝 stale journal。coordinator 在 privileged call 前先持久化 adapter/generation binding，返回 handle 后继续校验 adapter/platform/spec/generation/config revision/helper PID/完整 artifact lineage；被拒绝的 untrusted receipt 不会进入 privileged `stop(handle)`，只允许已绑定 adapter 做 generation-scoped rollback。磁盘 Store 使用 WAL + `synchronous=FULL`，内存测试库保持 `NORMAL`。Store 在打开 SQLite 前 canonicalize 并持有 app-data 目录，复核目录/锁/DB 路径身份并持续持有 OS owner lock；Unix 收紧目录/锁权限并拒绝 symlink/hardlink，Windows source 使用禁止 delete-sharing 的目录/锁 handle、reparse 拒绝和 volume/File ID 比较。它是当前 source-level journal owner 边界，Tauri single-instance 插件仅负责 activation UX；已有 same-process double-open/drop-release 与 Unix path 测试，但 handle-relative SQLite/VFS、Windows SID/DACL、恶意 same-user substitution、三平台真实跨进程/crash/multi-session 仍待完成。Linux 包边界进一步加入 root runtime-dir 精确权限、helper/package-manager 共享锁、崩溃持久 sentinel、严格 staging、输入快照、hermetic verifier、DEB/RPM 语义元数据拒绝与确定性 mode；GitHub Actions 使用完整 commit SHA，Rust Gate 固定到 `1.95.0`。当前 workflow 明确为 Source/Quick Non-Release；Windows/macOS source compile/process checks 已配置，但取得对应 CI runner 结果前不能声称编译通过，正式 `release.yml` 也尚未依赖它。实际 MSRV 编译发现 `sysinfo 0.39` 明示要求 1.95，`libsqlite3-sys 0.38.1` 构建脚本也使用 1.95 稳定的 `cfg_select!`，故将原先无法兑现的 1.94 声明同步提升，避免为保留虚假 MSRV 回退进程与数据库依赖。所有入口继续 fail-closed：默认 orchestrator 未注入真实 adapter，Linux capability 全 false，architecture/OpenPGP/完整 distro dependency contract 与受保护 lab attestation 仍为 `unconfigured`。目前仍无具体 TCP/UDP/reassembly/Virtual-DNS provider、可信 tuple→profile side channel或真实 root/native/performance/长稳证据，因此不能称为任一平台已接通或生产就绪。
...
| 共享数据面 | bounded ingress/L3 admission、`PacketStackSupervisor`、`FlowRuntime` 与唯一公开 `ProductDataPlaneSupervisor`；generation registry 在 caller cancel 后仍保留 startup/recovery owner，Linux 不再复制私有 registry；root-cause diagnostics 与 cleanup proof 独立，只有 stack/runtime/profile/task owner 都实际 join 才能 clean | 仍缺具体受控 TCP/UDP/reassembly/Virtual-DNS `PacketStackProvider`、最终 profile/config builder和三平台真实 native ingress；这些仍是 source/memory contract，不能解锁 capability 或替代 native/performance/24h/7d Gate |
| Linux 特权、产品 owner 与包 | cgroup v2+nft+fwmark/TUN、认证 helper、WAL/PID、fixed TUN/pump/lifecycle、generation adapter/coordinator recovery；DEB/RPM-only overlay、policy/polkit、root:root 0755 runtime-dir、shared/exclusive lifecycle lock、package sentinel、严格 stage、快照/hermetic package verifier和元数据/mode Gate | signer/architecture、完整 GUI/系统动态依赖 profile与 trusted lab attestation 均 `unconfigured`；AppImage/updater capture-disabled；尚缺具体 provider/tuple side channel/default async injection、签名包及真实 package-manager/root/native/PID-race lab，因此 capability 全 false |
...
- 当前已落地并硬化 bounded decoded-stream 合同：`FlowDescriptor` 对 generation、平台、五元组、PID/start token、app identity、capture intent 与 profile binding fail closed；只有明确 global capture 可用 global fallback，app/PID 归因缺失直接拒绝。`FlowEngine` 绑定 revision 与完整 profile SHA-256；`FlowRuntime` 持有 admission/task/egress/profile cleanup state，root-cause diagnostic 与当前 cleanup proof 分离，只有所有 live owner 的完成都被观察到才报告 clean。Linux source bridge已接入该公共composition seam，但没有具体provider，也未从默认 `AppState`/commands/tray启用。
...
- startup、ready前失败、runtime启动失败、stop timeout或调用者取消时，stack/runtime/profile/task owner均保留显式retry state；emergency reaper只作最后的资源上限，不是cleanup证明。provider terminal根因可以保留诊断，但不会在owner都已join后伪造residue；反之任何join不确定、live task或owner shutdown失败都不得被根因掩盖为clean。
...
| S1-STACK-SUPERVISOR（PARTIAL：公共 supervisor source complete） | `PacketStackSupervisor` 的 exact pin/capability/identity/first-terminal/one-shot handoff之上，唯一公开 `ProductDataPlaneSupervisor` 用 detached worker + generation registry 接管 startup；caller 在任意 await 点取消也能按 generation 取回并恢复 owner。`FlowRuntime` 将 root-cause diagnostic 与 cleanup-pending 分离，pending close/egress/profile/task/stack join 均保留 retry state，只有实际观察到 owner 完成才报告 clean | `flow/packet_stack.rs`、`flow/composition.rs`、`flow/runtime.rs`、`capture/packet_device.rs` | 仍须实现并固定具体受控 TCP/UDP/reassembly/Virtual-DNS provider、最终 profile/config builder并接入各平台 withdrawal；Windows/macOS 必须直接复用该 supervisor，禁止复制平台私有 registry；native/performance/24h/7d 前不得 PASS |
...
| S1-DNS-UDP（PARTIAL） | hostname source、unknown/degraded 语义以及 TCP-only egress 的 UDP BLOCK/DIRECT 决策已落；仍须冻结 DNS/Fake-IP attribution 并实现受控 UDP/Virtual-DNS runtime | `flow/attribution.rs`、现有 policy/egress contract；后续 DNS/UDP runtime 模块 | 当前静态策略不能证明真实流量不泄漏。IPv4/IPv6 必须使用同一策略；DoH/ECH 为 unknown 而非伪造域名；HTTP CONNECT/SSH 不把 UDP 标成已代理；真实 DNS/IPv4/IPv6/UDP leak matrix 未过前不得 PASS |
...
当前已完成 bounded ingress/L3 admission/supervisor、共享 product composition、Linux helper/client/TUN/pump/lifecycle、产品 generation owner/coordinator recovery 与包合同；尚未接入具体完整 stack provider或默认产品 capture runtime。不要把这些 focused tests或完整 Sockscap 软件测试 PASS，解释成真实 PacketIngress/IP-stack、UDP、
...
当前状态：**BLOCKED（选型已冻结，公共 composition 与 Linux 产品/包 source contract 已实现，发布证据未完成）**。策略、生命周期、source release gate、Linux privileged transaction/helper 认证底座、bounded ingress/FlowRuntime、strict IPv6 admission、packet-stack supervisor、唯一 product composition、Linux generation owner/coordinator recovery、DEB/RPM helper/polkit/package Gate 已落地。Windows source contract 固定为 Wintun global + WinDivert app/PID。具体 production TCP/UDP/reassembly/Virtual-DNS provider、三平台真实 vertical slice、配置后的签名/entitlement policy、签名 package/native/root/性能/长稳证据仍不存在；因此所有 capture capability 继续为 false。
...
当前状态：**PARTIAL**。DIRECT、SOCKS5 TCP、HTTP CONNECT、共享 SSH `direct-tcpip`、严格 host-key、取消/统计边界、公共 product supervisor和Linux adapter注入 seam 已经落地；尚缺具体 production TCP/UDP/reassembly/Virtual-DNS provider、最终 profile/config builder、默认 async 产品接线、DNS/UDP 闭环和三类真实服务器完整矩阵。
...
当前状态：**PARTIAL（公共 supervisor、产品 owner与包安全合同已落地，真实数据面仍 BLOCKED）**。既有 cgroup-v2/nft/fwmark/TUN、认证 helper、WAL/PID、fixed client/TUN、双 pump、fault withdrawal/tombstone之上，已新增取消安全的公共 generation supervisor、`LinuxCaptureAdapter`/coordinator recovery、Store-global detached operation mutex、disk WAL `synchronous=FULL`，以及 canonical directory/lock/DB checks + retained OS owner lock 的 source-level journal ownership 边界；desktop single-instance 仅负责 activation UX。当前只有 same-process double-open/drop-release 与 Unix path tests；handle-relative SQLite/VFS、Windows SID/DACL、三平台跨进程/crash/multi-session native、WAL fault/power-loss 仍待验。另有带生命周期锁/sentinel/快照/hermetic verifier/元数据拒绝的 DEB/RPM-only package Gate。仍缺具体 stack provider、最终 profile/config builder、tuple→profile side channel、默认产品注入/异步 IPC，以及配置后的 signer/architecture/完整依赖 policy、可信 lab attestation和真实 package-manager/root/native smoke，因此四个 capability 位全部保持 false。

- 完成 13.0.4 剩余项：选定并实现具体受控 stack provider与最终 Linux composition builder，接通 profile/config snapshot 和可信 tuple→profile side channel；在不改变 fail-closed probe 的前提下完成默认产品/异步 IPC 注入，再取得签名 package/native/root evidence。
...
- 尚未实现或运行：具体 production IP-stack TCP/UDP/reassembly/Virtual DNS provider、最终 profile/config builder与可信 tuple→profile side channel；`AppState`/async commands/tray 的真实 Linux adapter start/update/heartbeat/stop注入；Windows/macOS真实adapter；single-instance 的 window-ready pending focus 与 allowlisted `--sockscap-auto-restore` activation-intent 转交；Linux完整distro依赖profile、签名repository/package及root/native evidence；typed real-capture producer；protected signed lab attestation；三平台真实capture/egress/leak/performance/100-cycle/24h/7d、tray/system smoke和安装/升级/卸载矩阵。已落的Linux source owner/coordinator/package verifier不能代替上述证据，缺口关闭前四个capability位不得改变。
