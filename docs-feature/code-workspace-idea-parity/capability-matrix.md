...
- **2026-09-26 P1 新卡增量：** [ED-PARITY-006 设计](project-replace-exclude-plan.md#ed-parity-006) / [参照与 fixture](references/ed-parity-006-reference.md#observed) / [完整 P2](handoff-p2-ed-parity-006.md)。IDEA 目标经用户定为 2026.2.3 / IU-262.10968.63（仅本卡）；已实采 F1-REPL-006 的 scope/mask 集合、Find 窗口 Delete 排除、Replace All 确认/取消零写入/提交、Undo Replace 确认（外部修改/dirty 冲突未采）。源码确认 G1 结果列表无法排除、G2 提交后结果陈旧、G3 输入框 Ctrl+Z 直接撤销磁盘替换；G4/G5 为体验差异。P1 规划完成，唯一板同卡 ready/planning_required=false，无开发 owner；零产品执行，三维结论仍为待验证，未关闭本场景差距。
