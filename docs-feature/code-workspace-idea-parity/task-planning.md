...
- 来源：REQ-01 / CW-SEARCH-001、CW-SHELL-002、CW-NAV-002；[P0 需求](overall-audit-plan-20260913.md#req-01)、[历史只读材料](../../claudedocs/code-workspace-idea-parity-backlog-find-focus.md)。
- 已知依据：旧 ED-FINDFOCUS-001 为 implemented；P3 已记录 Windows native/IME/zoom，仍列有真实 JDT LS 与屏幕阅读器缺口。该状态仅作历史事实，不继承为新卡状态。
- 本包边界：核对 Find 已交付实现与当前源码身份，只处理原报告明确未完成的真实 provider/可访问性要求及其必要焦点序列。已有重入修复不重复开发，屏幕阅读器是否属于目标要求由 P1 对照原 AC 确认。
- 生产 owner 候选（待核）：CodeMirrorHost.tsx、editorSearchPanel.ts、lspHyperlink.ts、语义导航 caller；默认只读，P1 仅在发现明确新差距时界定产品改动。
- IDEA/fixture（待核）：F0 Find 与 F2 Java provider；复用 references/find-focus-2026.2.2.md，核对目标平台、JDK/provider 和辅助技术可用性。
- ED-PARITY-001-A1：在真实 provider 可用时 Find 打开/匹配/Esc 后 modifier-hover 导航仍正确；适用的键盘、焦点和可访问性要求有明确观测或具体未验证结论。
- ED-PARITY-001-A2：同 fixture 的 IDEA/Taomni 功能、视觉、交互分别有结论和准确证据身份；缺侧或不支持明确标记，不能声称 matched。
- ED-PARITY-001-A3：保留已交付的无重入、当前匹配 selection、多 view、Replace/undo、dirty 与 IME 契约。
- 验证起点：V1→A1，V2→A2，V3→A3；所需种类为 `document`、`code-audit`、`native`、`provider`、`accessibility`、`idea-comparison`。具体 case/命令、环境和可复用证据由 P1 核对后写入；本次均未执行。
