...
**2026-09-15 接续修订：** 首次 P0 已完成；本文件的首次事实和首包建议保留历史日期。
后续统一使用 [P0 新任务板](backlog.md) 和 [固定 P1](handoff-p1-next.md)，不从历史板领取。
本次补录独立新卡，P1 只选定一张卡做规划，开发交给 P2；既有 Tree/Shell/Rename 首包不重复建卡。
下文“尚未创建任务卡”“尚不可领取”等是当时记录，当前身份与映射看 [index](index.md)，不据此重跑首次 P0。
